...
class qc_sld_user_list {
	...
	public function update_action() {
		if ( isset( $_GET['action'] ) && in_array( $_GET['action'], array( 'approve', 'deny' ) ) && !isset( $_GET['new_role'] ) ) {
			...
			$user = absint( sanitize_text_field($_GET['user']) );
	...
	public function user_table_actions( $actions, $user ) {
		...
		$approve_link = wp_nonce_url( $approve_link, 'qc-opd' );

		$deny_link = add_query_arg( array( 'action' => 'deny', 'user' => $user->ID ) );
		$deny_link = remove_query_arg( array( 'new_role' ), $deny_link );
		$deny_link = wp_nonce_url( $deny_link, 'qc-opd' );

		$approve_action = '<a href="' . esc_url( $approve_link ) . '">' . __( 'Approve', 'qc-opd' ) . '</a>';
		$deny_action = '<a href="' . esc_url( $deny_link ) . '">' . __( 'Deny', 'qc-opd' ) . '</a>';
	...
	public function add_column( $columns ) {
		$the_columns['sld_user_status'] = __( 'Status', 'qc-opd' );
	...
	public function status_column( $val, $column_name, $user_id ) {
		switch ( $column_name ) {
			case 'sld_user_status' :
				...
				if ( $status == 'approved' ) {
					$status_i18n = __( 'approved', 'qc-opd' );
				} else if ( $status == 'denied' ) {
					$status_i18n = __( 'denied', 'qc-opd' );
				} else if ( $status == 'pending' ) {
					$status_i18n = __( 'pending', 'qc-opd' );
				}
				return $status_i18n;
				break;
	...
	public function status_filter( $which ) {
		...
		$filter_button = submit_button( __( 'Filter', 'qc-opd' ), 'button', 'pw-status-query-submit', false, array( 'id' => 'pw-status-query-submit' ) );
		$filtered_status = $this->selected_status();

		?>
		<label class="screen-reader-text" for="<?php echo $id ?>"><?php _e( 'View all users', 'qc-opd' ); ?></label>
		<select id="<?php echo $id ?>" name="<?php echo $id ?>" style="float: none; margin: 0 0 0 15px;">
			<option value=""><?php _e( 'View all users', 'qc-opd' ); ?></option>
	...
	private function selected_status() {
		if ( ! empty( $_REQUEST['qc_sld_filter-top'] ) || ! empty( $_REQUEST['qc_sld_filter-bottom'] ) ) {
			return esc_attr( ( ! empty( $_REQUEST['qc_sld_filter-top'] ) ) ? sanitize_text_field($_REQUEST['qc_sld_filter-top']) : sanitize_text_field($_REQUEST['qc_sld_filter-bottom']) );
		}
		...
		?>
			<script type="text/javascript">
				jQuery(document).ready(function ($){
					...
					$('<option>').val('approve').text('<?php _e( 'Approve', 'qc-opd' )?>').appendTo("select[name='action']");
					$('<option>').val('approve').text('<?php _e( 'Approve', 'qc-opd' )?>').appendTo("select[name='action2']");

					$('<option>').val('deny').text('<?php _e( 'Deny', 'qc-opd' )?>').appendTo("select[name='action']");
					$('<option>').val('deny').text('<?php _e( 'Deny', 'qc-opd' )?>').appendTo("select[name='action2']");
				});
			</script>
		<?php endif;
	}
	...
	public function bulk_action() {
		...
		if ( $screen->id == 'users' ) {
			...
			if ( isset( $_REQUEST['users'] ) ) {
				$user_ids = array_map( 'intval', sanitize_text_field($_REQUEST['users']) );
			}
	...
	public function admin_notices() {
		...
		if ( isset( $_REQUEST['denied'] ) && (int) $_REQUEST['denied'] ) {
			$denied = esc_attr( sanitize_text_field($_REQUEST['denied']) );
			$message = sprintf( _n( 'User denied.', '%s users denied.', $denied, 'qc-opd' ), number_format_i18n( $denied ) );
		}

		if ( isset( $_REQUEST['approved'] ) && (int) $_REQUEST['approved'] ) {
			$approved = esc_attr( sanitize_text_field( $_REQUEST['approved']) );
			$message = sprintf( _n( 'User approved.', '%s users approved.', $approved, 'qc-opd' ), number_format_i18n( $approved ) );
		}
	...
	public function profile_status_field( $user ) {
		...
		<table class="form-table">
			<tr>
				<th><label for="qc_sld_status"><?php _e( 'Access Status', 'qc-opd' ); ?></label>
				</th>
				<td>
					<select id="qc_sld_status" name="qc_sld_status">
						<?php if ( $user_status == 'pending' ) : ?>
							<option value=""><?php _e( '-- Status --', 'qc-opd' ); ?></option>
						<?php endif; ?>
					...
					<span
						class="description"><?php _e( 'If user has access to sign in or not.', 'qc-opd' ); ?></span>
					<?php if ( $user_status == 'pending' ) : ?>
						<br/><span
							class="description"><?php _e( 'Current user status is <strong>pending</strong>.', 'qc-opd' ); ?></span>
					<?php endif; ?>
				</td>
			</tr>
		</table>
	...
	public function pending_users_bubble() {
		...
		$query = array( 'meta_key' => 'sld_user_status', 'meta_value' => 'pending', );
