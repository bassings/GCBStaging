...
<?php wp_enqueue_style('sld-css-style-11-tooltip', SLD_OCOPD_TPL_URL . "/$template_code/css/darktooltip.css" ); ?>
<?php wp_enqueue_style('sld-css-style-11', SLD_OCOPD_TPL_URL . "/$template_code/template.css" ); ?>
<?php wp_enqueue_script('sld-css-style-11-tooltip-js', SLD_OCOPD_TPL_URL . "/$template_code/css/jquery.darktooltip.js"); ?>
...
<?php
	...
	$qcopd_tooltip_tpl11_custom_js = "";

	
	$qcopd_tooltip_tpl11_custom_js .= "jQuery(document).ready(function($){
...
wp_add_inline_script( 'qcopd-custom-script', $qcopd_tooltip_tpl11_custom_js);
...
if ( $list_query->have_posts() )
{
	...
	echo '<div class="qcld-main-container-style-11"><div class="qcopd-list-wrapper qc-full-wrapper">';
	?>
	<?php
...
//If FILTER TYPE is NORMAL
	...
	if(is_user_logged_in() and $sldfavorite=='on'){
		...
		$bookmark_list_custom_css .= "
			#bookmark_list.style-11 .ca-menu li {
			...
			#bookmark_list.style-11 .ca-menu li:hover .ca-main {
			  color: ".$conf['list_txt_color_hov'].";
			}

			#bookmark_list.style-11 .ca-menu li .ca-sub {
			...
			#bookmark_list.style-11 .ca-menu li:hover .ca-sub {
			  color: ".$conf['list_subtxt_color_hov'].";
			}

			#bookmark_list.style-11 .ca-menu li .upvote-section .upvote-btn, #qcopd-list-".$listId .'-'. get_the_ID()." .ca-menu li .upvote-section .upvote-count{
			  color: ".$conf['list_subtxt_color'].";
			}

			#bookmark_list.style-11 .ca-menu li:hover .upvote-section .upvote-btn, #bookmark_list.style-11 .ca-menu li:hover .upvote-section .upvote-count{
			  color: ".$conf['list_subtxt_color_hov'].";
			}";

        wp_add_inline_style( 'sld-css-style-11', $bookmark_list_custom_css );
...
?>
		...
		<div id="bookmark_list" class="qc-grid-item qcopd-list-column opd-column-<?php echo $column; echo " ". $style; echo " " . $new_class; ?> <?php echo "opd-list-id-" . get_the_ID(); ?>">
			...
			<div class="qcopd-single-list style-12-div">
				...
				<div>
			       <ul class="tpl-12-img-logo-list ca-menu column<?php echo $column; ?>" id="sld_bookmark_ul">
			...
			if(!empty($user_meta_data)){
				foreach($user_meta_data[0] as $postid=>$metaids){
					...
					<li id="item-<?php echo get_the_ID() ."-". $count; ?>" class="tooltip_tpl11 sld-style11-column<?php echo $column; ?>" data-tooltip="<?php echo $sub; ?>" style="<?php echo ( isset($list['list_item_bg_color']) && !empty($list['list_item_bg_color']) ) ? 'background:'. esc_attr($list['list_item_bg_color']) : ''; ?>">
						<?php
						...
						<?php else: ?>
						<a <?php if( $mask_url == 'on') { echo 'onclick="document.location.href = \''.$item_url.'\'; return false;"'; } ?>  <?php echo esc_attr($qcopd_item_nofollow); ?> href="<?php echo $masked_url; ?>" <?php echo (isset($list['qcopd_item_newtab']) && $list['qcopd_item_newtab'] == 1) ? 'target="_blank"' : ''; ?> data-title="<?php echo $sub; ?>" >
						<?php endif; ?>
							...
							<?php if( ($list_img == "true") && isset($list['qcopd_item_img'])  && $list['qcopd_item_img'] != "" ) : ?>
								...
								?>
								<img src="<?php echo $img[0]; ?>" alt="<?php echo trim( $image_alt_text ); ?>">
							<?php elseif( $showFavicon == 1 && $faviconFetchable == true ) : ?>
								<span class="icon logo-icon favicon-loaded">
									<img src="<?php echo $faviconImgUrl; ?>" alt="<?php echo trim($list['qcopd_item_title']); ?>">
								</span>
							...
							<?php else : ?>
								<img src="<?php echo SLD_QCOPD_IMG_URL; ?>/list-image-placeholder.png" alt="<?php echo trim($list['qcopd_item_title']); ?>">
							<?php endif; ?>
...
if($bookmark_list == "favorite" ){
	...
	while ( $list_query->have_posts() )
	{
		...
		$qcopd_list_custom_css .= "
			#item-".$listId .'-'. get_the_ID()."-add-block {
			  margin-top: 35px;
			  border-radius: 5px;
			...
			if( isset($conf['list_bg_featured_color']) && !empty(@$conf['list_bg_featured_color'])){
				$qcopd_list_custom_css .= ".qcopd-list-wrapper .style-11 .featured-section {
        ...
        wp_add_inline_style( 'sld-css-style-11', $qcopd_list_custom_css );
		?>
		...
		<?php if( !empty($lists) && (count($lists) > -1) ){ ?>
			<div id="qcopd-list-<?php echo $listId .'-'. get_the_ID(); ?>" class="qc-grid-item qcopd-list-column <?php echo $style; ?> <?php echo "opd-list-id-" . get_the_ID(); ?>">
				...
				<div class="qcopd-single-list style-12-div">
					...
					<?php } ?>
					<div>
				       <ul class="tpl-12-img-logo-list ca-menu column<?php echo $column; ?>" id="jp-list-<?php echo $SLD_QCOPD_DIRectory_instance_count.'-'.get_the_ID(); ?><?php echo (isset($cattabid)&&$cattabid!=''?'-'.$cattabid:''); ?>">
						<?php $count = 1; 
						if( $item_orderby == 'upvotes' )
							{
								if( $item_order == 'ASC' ){
						...
						<li id="item-<?php echo get_the_ID() ."-". $count; ?>" class="tooltip_tpl11 sld-style11-column<?php echo $column; ?>" data-tooltip="<?php echo $sub; ?>" style="<?php echo ( isset($list['list_item_bg_color']) && !empty($list['list_item_bg_color']) ) ? 'background:'. esc_attr($list['list_item_bg_color']) : ''; ?>">
							...
							<?php if(sld_is_youtube_video($item_url)): ?>
								...
								<a title="<?php echo esc_attr($list['qcopd_item_title']); ?>" <?php echo $videoPopContent; ?> href="<?php echo esc_url($list['qcopd_item_link']); ?>" <?php if($video_click!='popup'){ echo (isset($list['qcopd_item_newtab']) && $list['qcopd_item_newtab'] == 1) ? 'target="_blank"' : ''; echo 'data-itemurl="'.$list['qcopd_item_link'].'"'; } ?> data-mfp-src="#sldvideo-<?php echo $count; ?>" data-itemid="<?php echo get_the_ID(); ?>" data-videourl="<?php echo $list['qcopd_item_link']; ?>" data-itemsid="<?php echo $list['qcopd_timelaps']; ?>" data-tag="<?php echo (isset($list['qcopd_tags'])?$list['qcopd_tags']:'' ); ?>"  data-subtitle="<?php echo esc_attr( $subtitle ); ?>">
							...
							<?php elseif(sld_is_vimeo_video($item_url)): ?>
								...
								<a title="<?php echo esc_attr($list['qcopd_item_title']); ?>" <?php echo $videoPopContent; ?> href="<?php echo $list['qcopd_item_link']; ?>" <?php if($video_click!='popup'){ echo (isset($list['qcopd_item_newtab']) && $list['qcopd_item_newtab'] == 1) ? 'target="_blank"' : ''; echo 'data-itemurl="'.$list['qcopd_item_link'].'"'; } ?> data-mfp-src="#sldvideo-<?php echo $count; ?>" data-itemid="<?php echo get_the_ID(); ?>" data-videourl="<?php echo $list['qcopd_item_link']; ?>" data-itemsid="<?php echo $list['qcopd_timelaps']; ?>" data-subtitle="<?php echo esc_attr( $subtitle ); ?>" >
							...
							?>
								...
								<?php echo (isset($list['qcopd_item_newtab']) && $list['qcopd_item_newtab'] == 1) ? 'target="_blank"' : ''; ?> data-itemid="<?php echo get_the_ID(); ?>" data-itemurl="<?php echo $list['qcopd_item_link']; ?>" data-itemsid="<?php echo $list['qcopd_timelaps']; ?>" data-tag="<?php echo (isset($list['qcopd_tags'])?$list['qcopd_tags']:'' ); ?>" data-subtitle="<?php echo esc_attr( $subtitle ); ?>" <?php echo $popContent; ?>>
