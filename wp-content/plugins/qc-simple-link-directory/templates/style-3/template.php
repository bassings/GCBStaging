<?php wp_enqueue_style('sld-css-style-3', SLD_OCOPD_TPL_URL . "/$template_code/template.css" ); ?>
...
if ( $list_query->have_posts() )
{
	...
	echo '<div class="qcld-main-container-style-3"><div class="qcopd-list-wrapper qc-full-wrapper '. $subtitle_present_class . '">';
	?>
	<?php
	//if(sld_get_option('sld_enable_filtering_left')=='on' || $enable_left_filter=='true') {
    if( ( sld_get_option('sld_enable_filtering_left')=='on' && $enable_left_filter !='false'  ) || $enable_left_filter=='true') {
...
?>
		...
		<div id="bookmark_list" class="<?php echo $style;  echo " " . $new_class; ?>">

			<div class="qcopd-single-list">
				
				<h2><?php echo ($b_title!=''?$b_title:'Quick Links'); ?></h2>
				<ul class="ca-menu column<?php echo $column; ?>" id="sld_bookmark_ul">
					<?php $lists = array();
			if(!empty($user_meta_data)){
				foreach($user_meta_data[0] as $postid=>$metaids){
					...
					?>
					<li id="sld_bookmark_li_<?php echo $b; ?>" class="column<?php echo $column; ?>" style="<?php echo ( isset($list['list_item_bg_color']) && !empty($list['list_item_bg_color']) ) ? 'background:'. esc_attr($list['list_item_bg_color']) : ''; ?>">
						<?php
						...
						?>
							...
							<?php echo (isset($list['qcopd_item_newtab']) && $list['qcopd_item_newtab'] == 1) ? 'target="_blank"' : ''; ?> data-itemid="<?php echo esc_attr($list['postid']); ?>" data-itemurl="<?php echo esc_url($list['qcopd_item_link']); ?>" data-itemsid="<?php echo esc_attr($list['qcopd_timelaps']); ?>" data-tag="<?php echo (isset($list['qcopd_tags'])?esc_attr($list['qcopd_tags']):'' ); ?>" <?php echo $popContent; ?>  data-subtitle="<?php echo esc_attr( $subtitle ); ?>">
...
//adding extra variable in config
        ...
        $qcopd_list_custom_css .= "
			#qcopd-list-".$listId .'-'. get_the_ID().".style-3 .ca-menu li {
			    background-color: ".$conf['list_bg_color'].";
				box-shadow: 1px 1px 2px ".$conf['item_bdr_color'].";
				border-top: 6px solid ".$conf['list_txt_color'].";
			}

			#qcopd-list-".$listId .'-'. get_the_ID().".style-3 .ca-menu li:hover {
			    background-color: ".$conf['list_bg_color_hov'].";
				color: ".$conf['list_txt_color_hov'].";
				box-shadow: 1px 1px 2px ".$conf['item_bdr_color_hov'].";
				border-top: 6px solid ".$conf['list_txt_color_hov'].";
			}

			#qcopd-list-".$listId .'-'. get_the_ID().".style-3 .ca-menu li .ca-main {
			...
			#qcopd-list-".$listId .'-'. get_the_ID().".style-3 .ca-menu li:hover .ca-main {
			  color: ".$conf['list_txt_color_hov'].";
			}

			#qcopd-list-".$listId .'-'. get_the_ID().".style-3 .ca-menu li .ca-sub {
			...
			#qcopd-list-".$listId .'-'. get_the_ID().".style-3 .ca-menu li:hover .ca-sub {
			  color: ".$conf['list_subtxt_color_hov'].";
			}

			#qcopd-list-".$listId .'-'. get_the_ID().".style-3 .ca-menu li:hover .upvote-section {
			  color:  ".$conf['list_subtxt_color_hov'].";
			}

			#qcopd-list-".$listId .'-'. get_the_ID().".style-3 .ca-menu li .upvote-section .upvote-btn, #qcopd-list-".$listId .'-'. get_the_ID().".style-3 .ca-menu li .upvote-section .upvote-count{
			  color: ".$conf['list_subtxt_color'].";
			}

			#qcopd-list-".$listId .'-'. get_the_ID().".style-3 .ca-menu li:hover .upvote-section .upvote-btn, #qcopd-list-".$listId .'-'. get_the_ID().".style-3 .ca-menu li:hover .upvote-section .upvote-count{
			  color: ".$conf['list_subtxt_color_hov'].";
			}
			...
			if( isset($conf['list_bg_featured_color']) && !empty(@$conf['list_bg_featured_color'])){
				$qcopd_list_custom_css .= ".qcopd-list-wrapper .style-3 .featured-section {
        ...
        wp_add_inline_style( 'sld-css-style-3', $qcopd_list_custom_css );
		...
		<?php if( !empty($lists) && (count($lists) > -1) ){ ?>
			<div id="qcopd-list-<?php echo $listId .'-'. get_the_ID(); ?>" class="<?php echo $style; ?>">
				...
				<div class="qcopd-single-list">
					...
					<?php } ?>
					<ul class="ca-menu column<?php echo $column; ?>" id="jp-list-<?php echo $SLD_QCOPD_DIRectory_instance_count.'-'.get_the_ID(); ?><?php echo (isset($cattabid)&&$cattabid!=''?'-'.$cattabid:''); ?>">
						<?php $count = 1; 
						if( $item_orderby == 'upvotes' )
							{
								if( $item_order == 'ASC' ){
						...
						?>
						<li id="item-<?php echo get_the_ID() ."-". $count; ?>" class="column<?php echo $column; ?>"  style="<?php echo ( isset($list['list_item_bg_color']) && !empty($list['list_item_bg_color']) ) ? 'background:'. esc_attr($list['list_item_bg_color']) : ''; ?>">
							...
							<?php
								...
								if($main_click=='popup'){
									$masked_url = '#';
									$popContent = 'class="open-mpf-sld-more2 sld_load_more2" data-post-id="'.get_the_ID().'" data-item-title="'.esc_attr(trim($list['qcopd_item_title'])).'" data-item-link="'.esc_url($list['qcopd_item_link']).'" data-mfp-src="#sldinfo-'.get_the_ID() ."-". $count."-". esc_attr($list['qcopd_timelaps']).'"';
								}
							...
							<?php if(sld_is_youtube_video($item_url)): ?>
								...
								<a <?php echo $videoPopContent; ?> title="<?php echo esc_attr($list['qcopd_item_title']); ?>" href="<?php echo esc_url($list['qcopd_item_link']); ?>" <?php if($video_click!='popup'){ echo (isset($list['qcopd_item_newtab']) && $list['qcopd_item_newtab'] == 1) ? 'target="_blank"' : ''; echo 'data-itemurl="'.$list['qcopd_item_link'].'"'; } ?> data-mfp-src="#sldvideo-<?php echo $count; ?>" data-itemid="<?php echo get_the_ID(); ?>" data-videourl="<?php echo esc_url($list['qcopd_item_link']); ?>" data-itemsid="<?php echo $list['qcopd_timelaps']; ?>" data-tag="<?php echo (isset($list['qcopd_tags'])?esc_attr($list['qcopd_tags']):'' ); ?>" data-subtitle="<?php echo esc_attr( $subtitle ); ?>">
							...
							<?php else: ?>
								...
								<a title="<?php echo esc_attr($list['qcopd_item_title']); ?>" <?php if( $mask_url == 'on') { echo 'onclick="document.location.href = \''.$item_url.'\'; return false;"'; } ?> <?php echo esc_attr($qcopd_item_nofollow); ?> href="<?php echo esc_url($masked_url); ?>"
								<?php echo (isset($list['qcopd_item_newtab']) && $list['qcopd_item_newtab'] == 1) ? 'target="_blank"' : ''; ?> data-itemid="<?php echo get_the_ID(); ?>" data-itemurl="<?php echo esc_url($list['qcopd_item_link']); ?>" data-itemsid="<?php echo $list['qcopd_timelaps']; ?>" data-tag="<?php echo (isset($list['qcopd_tags'])?esc_attr($list['qcopd_tags']):'' ); ?>" <?php echo $popContent; ?>  data-subtitle="<?php echo esc_attr( $subtitle ); ?>">
						...
						<div class="style-3-upvote-section">
								...
								<div class="bookmark-section bookmark-section-style-3">
									...
									<span class="open-mpf-sld-more sld_load_more" data-post-id="<?php echo get_the_ID(); ?>" data-item-title="<?php echo esc_html(trim($list['qcopd_item_title'])); ?>" data-item-link="<?php echo esc_url($list['qcopd_item_link']); ?>" style="cursor:pointer" data-mfp-src="#sldinfo-<?php echo get_the_ID() ."-". $count ."-". esc_attr($list['qcopd_timelaps']); ?>">
