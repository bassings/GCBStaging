	...
	if ( ! class_exists( 'FusionSC_Progressbar' ) ) {
		...
		class FusionSC_Progressbar extends Fusion_Element {
			...
			public function render( $args, $content = '' ) {
				...
				$value_wrap     = 'yes' === $this->args['show_percentage'] ? ' <span ' . FusionBuilder::attributes( 'fusion-progressbar-value' ) . '>' . $value . $this->args['unit'] . '</span>' : '';
				...
				// Maximum value.
				$maximum_value_text         = '<span class="awb-pb-max-value-text">' . esc_html( $this->args['maximum_value_text'] ) . '</span>';
				$maximum_value              = 'percentage' === $this->args['value_display_type'] ? '100' : $this->convert_html_string_to_float( $this->args['maximum_value'] );
				$maximum_value_wrap         = 'yes' === $this->args['display_maximum_value'] ? ' <span class="awb-pb-max-value">' . esc_html( $maximum_value ) . esc_html( $this->args['maximum_value_unit'] ) . '</span>' : '';
			...
			[
				...
				'params'        => [
					...
					[
						...
						'description'  => esc_attr__( 'Use an integer or float here to denote the maximum value of the progress bar. The value set here will serve as the 100% meassure.', 'fusion-builder' ),
						'dynamic_data' => true,
