	...
	jQuery( document ).ready( function() {
		...
		FusionPageBuilder.fusion_imageframe = FusionPageBuilder.ElementView.extend( {
			...
			buildAttr: function( values ) {
				...
				attr = _.awbHTMLAttributes( values, attr );
			...
			buildContentAttr: function( values ) {
				...
				contentAttr = _.awbHTMLAttributes( values, contentAttr, 'main_node' );
			...
			isImgUrl( url ) {
				if ( url.includes( 'gravatar.com' ) || url.includes( 'unsplash.com' ) || url.includes( 'data:image' ) ) {
					return true;
				}
