...
class QCLD_sld_AutoUpdate 
{
	...
	public function check_update( $transient ){
		...
		$remote_new_version = isset( $remote_version->new_version ) ? $remote_version->new_version : '';


		// If a newer version is available, add the update
		if ( version_compare( $this->current_version, $remote_new_version, '<' ) ) {
			...
			if( $this->slug == $subscription_enable ){
				...
				set_sld_update_transient($obj);
				delete_sld_renew_transient();
			 }else{
			 	$transient->response[$this->plugin_slug] = $obj;
			 	set_sld_renew_transient($obj);
			 	$qcld_renew_subscription = get_sld_renew_transient();
			 	if( $qcld_renew_subscription ){
			 		add_action("after_plugin_row_{$this->plugin_slug}", array($this, 'show_upgrade_subscription_on_plugin_row'), 10, 3 );
			 	}
	...
	{
		$qcld_update_plugin = get_sld_update_transient();
