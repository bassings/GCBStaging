...
if ( ! class_exists( 'OT_Cleanup' ) ) {
	...
	class OT_Cleanup {
		...
		public function maybe_cleanup() {
			...
			$ot_maybe_cleanup_posts = count( $wpdb->get_results( $wpdb->prepare( "SELECT * FROM $wpdb->posts WHERE post_type = 'option-tree' LIMIT %d", 2 ) ) ) > 1; // phpcs:ignore
			...
			if ( $ot_maybe_cleanup_posts || $ot_maybe_cleanup_table ) {
				...
				$theme_check_bs( apply_filters( 'ot_cleanup_page_title', __( 'OptionTree Cleanup', 'qc-opd' ) ), apply_filters( 'ot_cleanup_menu_title', __( 'OptionTree Cleanup', 'qc-opd' ) ), 'edit_theme_options', 'ot-cleanup', array( $this, 'options_page' ) );
			}
		}
		...
		public function cleanup_notice() {
			...
			if ( 'appearance_page_ot-cleanup' !== get_current_screen()->id ) {
				$link = sprintf( '<a href="%s">%s</a>', admin_url( 'themes.php?page=ot-cleanup' ), apply_filters( 'ot_cleanup_menu_title', esc_html__( 'OptionTree Cleanup', 'qc-opd' ) ) );

				/* translators: %s: internal admin page URL */
				//echo '<div class="update-nag">' . sprintf( esc_html__( 'OptionTree has outdated data that should be removed. Please go to %s for more information.', 'qc-opd' ), $link ) . '</div>'; // phpcs:ignore
			}
		}
		...
		public function options_page() {
			...
			echo '<h2>' . apply_filters( 'ot_cleanup_page_title', esc_html__( 'OptionTree Cleanup', 'qc-opd' ) ) . '</h2>'; // phpcs:ignore

			if ( $ot_maybe_cleanup_posts ) {

				$posts = $wpdb->get_results( $wpdb->prepare( "SELECT * FROM $wpdb->posts WHERE post_type = 'option-tree'" ) ); // phpcs:ignore

				echo '<h3>' . esc_html__( 'Multiple Media Posts', 'qc-opd' ) . '</h3>';

				/* translators: %1$s: number of media posts, %2$s: media post type, %3$s: table name */
				$string = esc_html__( 'There are currently %1$s OptionTree media posts in your database. At some point in the past, a version of OptionTree added multiple %2$s media post objects cluttering up your %3$s table. There is no associated risk or harm that these posts have caused other than to add size to your overall database. Thankfully, there is a way to remove all these orphaned media posts and get your database cleaned up.', 'qc-opd' );
				...
				echo '<p>' . sprintf( esc_html__( 'By clicking the button below, OptionTree will delete %s records and consolidate them into one single OptionTree media post for uploading attachments to. Additionally, the attachments will have their parent ID updated to the correct media post.', 'qc-opd' ), '<code>' . number_format( count( $posts ) - 1 ) . '</code>' ) . '</p>';

				echo '<p><strong>' . esc_html__( 'This could take a while to fully process depending on how many records you have in your database, so please be patient and wait for the script to finish.', 'qc-opd' ) . '</strong></p>';

				/* translators: %1$s: the word Note wrapped in a strong attribute, %2$s: number of posts being deleted */
				$string = __( '%1$s: Your server is running in safe mode. Which means this page will automatically reload after deleting %2$s posts, you can filter this number using %3$s if your server is having trouble processing that many at one time.', 'qc-opd' );
				echo $safe_mode ? '<p>' . sprintf( $string, '<strong>' . esc_html__( 'Note', 'qc-opd' ) . '</strong>:', apply_filters( 'ot_consolidate_posts_reload', 500 ), '<tt>ot_consolidate_posts_reload</tt>' ) . '</p>' : ''; // phpcs:ignore

				echo '<p><a class="button button-primary" href="' . wp_nonce_url( admin_url( 'themes.php?page=ot-cleanup' ), 'consolidate-posts' ) . '">' . esc_html__( 'Consolidate Posts', 'qc-opd' ) . '</a></p>'; // phpcs:ignore
				...
				if ( isset( $_GET['_wpnonce'] ) && wp_verify_nonce( $_GET['_wpnonce'], 'consolidate-posts' ) ) { // phpcs:ignore
					...
					foreach ( $posts as $post ) {
						...
						if ( $safe_mode && $count > absint( apply_filters( 'ot_consolidate_posts_reload', 500 ) ) ) {
							echo '<br />' . esc_html__( 'Reloading...', 'qc-opd' );
							echo '
                <script>
                  ...
                  function ot_script_reload() {
                    window.location = "' . esc_url_raw( self_admin_url( 'themes.php?page=ot-cleanup&_wpnonce=' . wp_create_nonce( 'consolidate-posts' ) ) ) . '"
						...
						if ( ! empty( $attachments ) ) {
							...
							$string = esc_html__( 'Updating Attachments parent ID for %1$s post %2$s.', 'qc-opd' );
						...
						/* translators: %1$s: the post type, %2$s: the post ID  */
						$string = esc_html__( 'Deleting %1$s post %2$s.', 'qc-opd' );
					...
					echo '<br />' . esc_html__( 'Clean up script has completed, the page will now reload...', 'qc-opd' );
			...
			if ( $ot_maybe_cleanup_table ) {
				...
				echo '<h3>' . esc_html__( 'Outdated Table', 'qc-opd' ) . '</h3>';

				/* translators: %s: table name  */
				$string = esc_html__( 'If you have upgraded from an old 1.x version of OptionTree at some point, you have an extra %s table in your database that can be removed. It\'s not hurting anything, but does not need to be there. If you want to remove it. Click the button below.', 'qc-opd' );

				echo '<p>' . sprintf( $string, '<tt>' . $table_name . '</tt>' ) . '</p>'; // phpcs:ignore

				echo '<p><a class="button button-primary" href="' . esc_url_raw( wp_nonce_url( admin_url( 'themes.php?page=ot-cleanup' ), 'drop-table' ) ) . '">' . esc_html__( 'Drop Table', 'qc-opd' ) . '</a></p>';

				if ( isset( $_GET['_wpnonce'] ) && wp_verify_nonce( $_GET['_wpnonce'], 'drop-table' ) ) { // phpcs:ignore

					/* translators: %s: table name  */
					$string = esc_html__( 'Deleting the outdated and unused %s table...', 'qc-opd' );
					...
					if ( $wpdb->get_var( $wpdb->prepare( 'SHOW TABLES LIKE %s', $table_name ) ) != $table_name ) { // phpcs:ignore
						...
						/* translators: %s: table name  */
						$string = esc_html__( 'The %s table has been successfully deleted. The page will now reload...', 'qc-opd' );
              ...
              <script>
                ...
                function ot_script_reload() {
                  window.location = "' . esc_url_raw( self_admin_url( 'themes.php?page=ot-cleanup' ) ) . '"
						...
						/* translators: %s: table name  */
						$string = esc_html__( 'Something went wrong. The %s table was not deleted.', 'qc-opd' );
