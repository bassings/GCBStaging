			...
			foreach ( (array) $this->options as $option ) {
				...
				foreach ( (array) $this->get_pages( $option ) as $page ) {
					...
					if ( ! isset( $page['hidden_page'] ) && $screen->id === $this->page_hook[ $page['id'] ] ) {
						...
						//$link = '<a href="https://wordpress.org/plugins/option-tree/" target="_blank">' . esc_html__( 'OptionTree', 'qc-opd' ) . '</a>';
						//echo '<li id="option-tree-logo">' . wp_kses_post( apply_filters( 'ot_header_logo_link', $link, $page['id'] ) ) . '</li>';

						echo '<li id="option-tree-version"><span>' . wp_kses_post( apply_filters( 'ot_header_version_text', 'OptionTree ' . OT_VERSION, $page['id'] ) ) . '</span></li>';
						...
						$this->do_settings_sections( isset( $_GET['page'] ) ? sanitize_text_field($_GET['page']) : '' ); // phpcs:ignore
						...
						if ( $show_buttons ) {
							...
							echo '<button type="submit" class="option-tree-ui-button button button-secondary left reset-settings" title="' . esc_html__( 'Reset Options', 'qc-opd' ) . '">' . esc_html__( 'Reset Options', 'qc-opd' ) . '</button>';
