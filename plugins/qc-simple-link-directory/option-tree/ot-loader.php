...
if ( class_exists( 'OT_Loader' ) && defined( 'OT_PLUGIN_MODE' ) && true === OT_PLUGIN_MODE && defined( 'ABSPATH' ) ) {
	...
	function ot_conflict_notice() {
		echo '<div class="error"><p>' . esc_html__( 'OptionTree is installed as a plugin and also embedded in your current theme. Please deactivate the plugin to load the theme dependent version of OptionTree, and remove this warning.', 'qc-opd' ) . '</p></div>';
	}
...
<style>
	...
	#adminmenu #toplevel_page_ot-settings .menu-icon-generic div.wp-menu-image:before {
		...
		public function add_section() {
			...
			$count  = isset( $_REQUEST['count'] ) ? absint( sanitize_text_field( $_REQUEST['count'] ) ) : 0;
		...
		public function change_image_button( $translation, $text, $domain ) {
			...
			if ( apply_filters( 'ot_theme_options_parent_slug', 'themes.php' ) === $pagenow && 'default' === $domain && 'Insert into post' === $text ) {
				...
				return apply_filters( 'ot_upload_text', esc_html__( 'Send to OptionTree', 'qc-opd' ) );
