...
<?php wp_enqueue_style('sld-css-style-8', SLD_OCOPD_TPL_URL . "/$template_code/template.css" ); ?>
...
if ( $list_query->have_posts() )
{
	...
	echo '<div class="qcld-main-container-style-8"><div class="qcopd-list-wrapper '. $subtitle_present_class . ' qc-full-wrapper">';
	?>
	<?php
	//if(sld_get_option('sld_enable_filtering_left')=='on' || $enable_left_filter=='true') {
    if( ( sld_get_option('sld_enable_filtering_left')=='on' && $enable_left_filter !='false'  ) || $enable_left_filter=='true') {
	...
	if(is_user_logged_in() and $sldfavorite=='on'){
		...
		$bookmark_list_custom_css .= "
			...
			#bookmark_list.style-8 li a {
			    ...
			    border-color: -moz-use-text-color -moz-use-text-color ".$conf['item_bdr_color']." !important;
			}

			#bookmark_list.style-8 li a:hover {
			    background-color: ".$conf['list_bg_color_hov'].";
				color: ".$conf['list_txt_color_hov'].";
				border-color: -moz-use-text-color -moz-use-text-color ".$conf['list_bg_color_hov']." !important;
			}
			...
			#bookmark_list.style-8 .tooltip_tpl8:before {
			 ...
			 $bookmark_list_custom_css .= " background-color: ".$conf['list_subtxt_color_hov'].";
			  border: 1px solid;
			  border-color: ".$conf['list_txt_color_hov'].";
			}

			#bookmark_list.style-8 .tooltip_tpl8:after {
			  border-bottom: 6px solid ".$conf['list_txt_color_hov'].";
			}

			#bookmark_list.style-8 li .upvote-section .upvote-btn, #qcopd-list-".$listId .'-'. get_the_ID().".style-8 .ca-menu li .upvote-section .upvote-count{
			  color: ".$conf['list_subtxt_color'].";
			}

			#bookmark_list.style-8 li:hover .upvote-section .upvote-btn, #qcopd-list-".$listId .'-'. get_the_ID().".style-8 li:hover .upvote-section .upvote-count{
			  color: ".$conf['list_subtxt_color_hov'].";
			}";

        wp_add_inline_style( 'sld-css-style-8', $bookmark_list_custom_css );
...
//adding extra variable in config
		...
		$qcopd_list_custom_css .= "
			#qcopd-list-".$listId .'-'. get_the_ID().".style-8 h3:after{
				border-bottom: 2px solid ".$conf['item_bdr_color'].";
			}

			#qcopd-list-".$listId .'-'. get_the_ID().".style-8 li > a {
			    background-color: ".$conf['list_bg_color'].";
			    border-color: -moz-use-text-color -moz-use-text-color ".$conf['item_bdr_color']." !important;
			}

			#qcopd-list-".$listId .'-'. get_the_ID().".style-8 li a:hover {
			    background-color: ".$conf['list_bg_color_hov'].";
				color: ".$conf['list_txt_color_hov'].";
				border-color: -moz-use-text-color -moz-use-text-color ".$conf['list_bg_color_hov']." !important;
			}

			#qcopd-list-".$listId .'-'. get_the_ID().".style-8 li a .li-txt {
			  color: ".$conf['list_txt_color'].";";
			...
			#qcopd-list-".$listId .'-'. get_the_ID().".style-8 li a:hover .li-txt {
			  color: ".$conf['list_txt_color_hov'].";

			}

			#qcopd-list-".$listId .'-'. get_the_ID().".style-8 .tooltip_tpl8:before {
			  color: ".$conf['list_subtxt_color'].";";
			...
			#qcopd-list-".$listId .'-'. get_the_ID().".style-8 .tooltip_tpl8:after {
			  border-bottom: 6px solid ".$conf['list_subtxt_color_hov'].";
			}

			#qcopd-list-".$listId .'-'. get_the_ID().".style-8 li .upvote-section .upvote-btn, #qcopd-list-".$listId .'-'. get_the_ID().".style-8 .ca-menu li .upvote-section .upvote-count{
			  color: ".$conf['list_subtxt_color'].";
			}

			#qcopd-list-".$listId .'-'. get_the_ID().".style-8 li:hover .upvote-section .upvote-btn, #qcopd-list-".$listId .'-'. get_the_ID().".style-8 li:hover .upvote-section .upvote-count{
			  color: ".$conf['list_subtxt_color_hov'].";
			}
			...
			if( isset($conf['list_bg_featured_color']) && !empty(@$conf['list_bg_featured_color'])){
				$qcopd_list_custom_css .= ".qcopd-list-wrapper .style-8 .featured-section {
        ...
        wp_add_inline_style( 'sld-css-style-8', $qcopd_list_custom_css );
		?>
			...
			<div id="qcopd-list-<?php echo $listId .'-'. get_the_ID(); ?>" class="qc-grid-item qcopd-list-column opd-column-<?php echo $column; echo " " . $style;?> <?php echo "opd-list-id-" . get_the_ID(); ?>">
				...
				<div class="opd-list-style-8 qcopd-single-list">
					...
					<h2 <?php echo (isset($conf['list_title_color'])&&$conf['list_title_color']!=''?'style="color:'.$conf['list_title_color'].';"':''); ?>>
						...
						if($multipage=='true'):
						?>
							<a href="<?php echo esc_url( $current_url.'/'.get_post(get_the_ID())->post_name ); ?>">
						<?php 
						elseif(isset($conf['title_link']) && $conf['title_link']!=''):
						?>
							<a href="<?php echo esc_url($conf['title_link']); ?>" <?php echo (isset($conf['title_link_new_tab'])&&$conf['title_link_new_tab']==1?'target="_blank"':'') ?> >
						<?php 
						endif;
						?>
						...
						<?php
							...
							?>
								<span class="opd-item-count">(<?php echo esc_attr($item_count_disp); ?>)</span>
							<?php 
							}
						?>
						<?php 
					...
					<?php } ?>
					<ul class="tooltip_tpl8-tpl" id="jp-list-<?php echo $SLD_QCOPD_DIRectory_instance_count.'-'.get_the_ID(); ?><?php echo (isset($cattabid)&&$cattabid!=''?'-'.$cattabid:''); ?>">
						<?php $count = 1;?>
						...
						<li id="item-<?php echo get_the_ID() ."-". $count; ?>" style="<?php echo ( isset($list['list_item_bg_color']) && !empty($list['list_item_bg_color']) ) ? 'background:'. esc_attr($list['list_item_bg_color']) : ''; ?>">
							...
							<?php else: ?>
								...
								<?php echo (isset($list['qcopd_item_newtab']) && $list['qcopd_item_newtab'] == 1) ? 'target="_blank"' : ''; ?> data-itemid="<?php echo get_the_ID(); ?>" <?php echo $toolipsetup; ?> data-itemurl="<?php echo $list['qcopd_item_link']; ?>" data-itemsid="<?php echo $list['qcopd_timelaps']; ?>" data-tag="<?php echo (isset($list['qcopd_tags'])?$list['qcopd_tags']:'' ); ?>" <?php echo $popContent; ?> style="<?php echo ( isset($list['list_item_bg_color']) && !empty($list['list_item_bg_color']) ) ? 'background:'. esc_attr($list['list_item_bg_color']) : ''; ?>" data-subtitle="<?php echo esc_attr( $subtitle ); ?>">
