<?php wp_enqueue_style('sld-css-style-15', SLD_OCOPD_TPL_URL . "/$template_code/style.css" ); ?>
...
if ( $list_query->have_posts() )
{
	...
	echo '<div class="qcld-main-container-style-15"><div class="qcopd-list-wrapper '. $subtitle_present_class . ' qc-full-wrapper">';
	?>
...
?>
		...
		<?php  if($favorite_hide != "hide" ){ 
			...
			?>
		<div id="bookmark_list" class="qc-feature-container qc-grid-item qc-sld-single-item-11 qcpd-list-column <?php echo $style; echo " " . $new_class;?>">
              	
				<h2><?php echo ($b_title!=''?$b_title:'Quick Links'); ?></h2>
				<ul id="sld_bookmark_ul" class="ca-menu column<?php echo $column; ?>">
<?php
			$lists = array();
...
?>
                  ...
                  <li class="opt-column-0<?php echo $column; ?>" id="pd_bookmark_li_<?php echo $b; ?>" <?php echo $tooltip_content; ?>>
						...
						<?php elseif(sld_is_audio_url($list['qcopd_item_link'])): ?>
							<a title="<?php echo esc_attr($list['qcopd_item_title']); ?>" <?php echo esc_attr($qcopd_item_nofollow); ?> 
							 data-tag="<?php echo (isset($list['qcopd_tags'])?$list['qcopd_tags']:'' ); ?>" data-itemid="<?php echo $list['postid']; ?>" data-itemurl="<?php echo $list['qcopd_item_link']; ?>" data-itemsid="<?php echo $list['qcopd_timelaps']; ?>">
                        ...
                        <a title="<?php echo esc_attr($list['qcopd_item_title']); ?>" <?php if( $mask_url == 'on') { echo 'onclick="document.location.href = \''.$item_url.'\'; return false;"'; } ?> <?php echo esc_attr($qcopd_item_nofollow); ?> href="<?php echo $masked_url; ?>"
							<?php echo (isset($list['qcopd_item_newtab']) && $list['qcopd_item_newtab'] == 1) ? 'target="_blank"' : ''; ?> data-tag="<?php echo (isset($list['qcopd_tags'])?$list['qcopd_tags']:'' ); ?>" data-itemid="<?php echo $list['postid']; ?>" data-itemurl="<?php echo $list['qcopd_item_link']; ?>" data-itemsid="<?php echo $list['qcopd_timelaps']; ?>">
						<?php endif; ?>
                        <div class="qc-sld-main">
                          <div class="qc-feature-media image">
							...
							<!-- Image, If Present -->
							<div class="qc-feature-media-image-block">
						...
						<div id="sldinfo-<?php echo $list['postid'] ."-". $b ."-". esc_attr($list['qcopd_timelaps']); ?>" class="white-popup mfp-hide">
							<div class="sld_more_text">
								<?php echo esc_html__('Loading...', 'qc-opd'); ?>
							</div>
						</div>
	...
	while ( $list_query->have_posts() )
	{
		...
		$qcopd_list_custom_css .= "
			...
			if( isset($conf['list_bg_featured_color']) && !empty(@$conf['list_bg_featured_color'])){ 
				$qcopd_list_custom_css .= ".qcopd-list-wrapper .style-15 .featured-section {
        ...
        wp_add_inline_style( 'sld-css-style-15', $qcopd_list_custom_css );
		?>
			...
			<div id="qcopd-list-<?php echo $listId .'-'. get_the_ID(); ?>" class="qc-feature-container qc-grid-item qc-sld-single-item-11 qcpd-list-column <?php echo $style;?> <?php echo "opd-list-id-" . get_the_ID(); ?>">
				...
				<div class="opd-list-style-12 sld-container title_style_sld qcopd-single-list" style="<?php echo ($listId==1)?'margin-top:10px':''; ?>">
					...
					<ul class="ca-menu column<?php echo $column; ?>" id="jp-list-<?php echo $SLD_QCOPD_DIRectory_instance_count.'-'.get_the_ID(); ?><?php echo (isset($cattabid)&&$cattabid!=''?'-'.$cattabid:''); ?>">
	                  ...
	                  <li class="opt-column-0<?php echo $column; ?>" id="item-<?php echo get_the_ID() ."-". $count; ?>" <?php echo $tooltip_content; ?> data-title="<?php echo $list['qcopd_item_title']; ?>" data-subtitle="<?php echo $subtitle; ?>" data-url="<?php echo $list['qcopd_item_link']; ?>">
							<?php
								$item_url = $list['qcopd_item_link'];
								$masked_url = $list['qcopd_item_link'];
								$popContent = '';
						...
						<?php else: ?>
							...
							<a title="<?php echo esc_attr($list['qcopd_item_title']); ?>" <?php if( $mask_url == 'on') { echo 'onclick="document.location.href = \''.$item_url.'\'; return false;"'; } ?> <?php echo esc_attr($qcopd_item_nofollow); ?> href="<?php echo $masked_url; ?>" <?php echo (isset($list['qcopd_item_newtab']) && $list['qcopd_item_newtab'] == 1) ? 'target="_blank"' : ''; ?> data-tag="<?php echo (isset($list['qcopd_tags'])?$list['qcopd_tags']:'' ); ?>" <?php echo $popContent; ?> data-itemid="<?php echo get_the_ID(); ?>" data-itemurl="<?php echo $list['qcopd_item_link']; ?>" data-itemsid="<?php echo $list['qcopd_timelaps']; ?>"  data-subtitle="<?php echo esc_attr( $subtitle ); ?>">
						<?php endif; ?>
	                        ...
	                        <div class="qc-sld-main">
	                          <div class="qc-feature-media image">
	                            ...
	                            <div class="upvote-section">
									...
									?>
									<?php if(sld_get_option('sld_enable_bookmark')=='on'): ?>
	                                <span class="bookmark-btn bookmark-on" data-post-id="<?php echo get_the_ID(); ?>" data-item-code="<?php echo trim($list['qcopd_timelaps']); ?>" data-is-bookmarked="<?php echo ($bookmark); ?>" >
									...
									<?php if( $upvote == 'on' ) : ?>
	                                <div class="favourite">
	                                    <span data-post-id="<?php echo get_the_ID(); ?>" data-unique="<?php echo get_the_ID().'_'.($list['qcopd_timelaps']!=''?$list['qcopd_timelaps']:$count); ?>" data-item-title="<?php echo trim($list['qcopd_item_title']); ?>" data-item-link="<?php echo $list['qcopd_item_link']; ?>" class="sld-upvote-btn upvote-btn upvote-on">
	                          ...
	                          <div class="qc-sld-content">
	                            <h4 class="sld-title">
	                            	<?php if(sld_is_youtube_video($extra_video_url)): ?>
										...
										<a title="<?php echo esc_attr($list['qcopd_item_title']); ?>" <?php echo $videoPopContent; ?> href="<?php echo esc_url($masked_url); ?>" <?php if($video_click!='popup'){ echo (isset($list['qcopd_item_newtab']) && $list['qcopd_item_newtab'] == 1) ? 'target="_blank"' : ''; echo 'data-itemurl="'.esc_attr($extra_video_url).'"'; } ?> data-mfp-src="#sldvideo-<?php echo $count; ?>" data-itemid="<?php echo esc_attr(get_the_ID()); ?>"  data-videourl="<?php echo esc_url($extra_video_url); ?>" data-itemsid="<?php echo esc_attr($list['qcopd_timelaps']); ?>" data-tag="<?php echo (isset($list['qcopd_tags'])?esc_attr($list['qcopd_tags']):'' ); ?>" ><?php echo $list['qcopd_item_title']; ?></a>
									...
									<?php elseif(sld_is_vimeo_video($extra_video_url)): ?>
										...
										<a title="<?php echo esc_attr($list['qcopd_item_title']); ?>" <?php echo $videoPopContent; ?> href="<?php echo esc_url($masked_url); ?>" <?php if($video_click!='popup'){ echo (isset($list['qcopd_item_newtab']) && $list['qcopd_item_newtab'] == 1) ? 'target="_blank"' : ''; echo 'data-itemurl="'.esc_url($extra_video_url).'"'; } ?> data-mfp-src="#sldvideo-<?php echo $count; ?>" data-itemid="<?php echo esc_attr(get_the_ID()); ?>" data-videourl="<?php echo esc_url($extra_video_url); ?>" data-itemsid="<?php echo esc_attr($list['qcopd_timelaps']); ?>" ><?php echo $list['qcopd_item_title']; ?></a>
										
									<?php else: ?>
	                            		<?php echo $list['qcopd_item_title']; ?>
	                            	<?php endif; ?>
	                            </h4>
