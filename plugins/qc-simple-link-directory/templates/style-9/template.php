...
<?php wp_enqueue_style('sld-css-style-9', SLD_OCOPD_TPL_URL . "/$template_code/template.css" ); ?>

<?php if( $tooltip == true ) : ?>
<?php wp_enqueue_style('sld-css-style-9-tooltip', SLD_OCOPD_TPL_URL . "/$template_code/html5tooltips.css" ); ?>
<?php wp_enqueue_style('sld-css-style-9-tooltip-animate', SLD_OCOPD_TPL_URL . "/$template_code/html5tooltips.animation.css" ); ?>
<?php wp_enqueue_script('jquery-style-9-jpages', SLD_OCOPD_TPL_URL . "/$template_code/html5tooltips.js"); ?>
...
if ( $list_query->have_posts() )
{
	...
	echo '<div class="qcld-main-container-style-9"><div class="qcopd-list-wrapper qc-full-wrapper">';
	?>
	<?php
	// if(sld_get_option('sld_enable_filtering_left')=='on' || $enable_left_filter=='true') {
    if( ( sld_get_option('sld_enable_filtering_left')=='on' && $enable_left_filter !='false'  ) || $enable_left_filter=='true') {
	...
	if(is_user_logged_in() and $sldfavorite=='on'){
		...
		$bookmark_list_custom_css .= "
			#bookmark_list.style-9 ul{
				border-top: 5px solid ".$conf['item_bdr_color'].";
        ...
        wp_add_inline_style( 'sld-css-style-9', $bookmark_list_custom_css );
			...
			if(!empty($user_meta_data)){
				foreach($user_meta_data[0] as $postid=>$metaids){
					...
					<li id="sld_bookmark_li_<?php echo $b; ?>" style="<?php echo ( isset($list['list_item_bg_color']) && !empty($list['list_item_bg_color']) ) ? 'background:'. esc_attr($list['list_item_bg_color']) : ''; ?>">
						...
						?>
							<a title="<?php echo esc_attr($list['qcopd_item_title']); ?>" <?php if( $mask_url == 'on') { echo 'onclick="document.location.href = \''.$item_url.'\'; return false;"'; } ?> <?php echo esc_attr($qcopd_item_nofollow); ?> href="<?php echo esc_url($masked_url); ?>" data-subtitle="<?php echo esc_attr($list['qcopd_item_subtitle']); ?>"
						...
						<a <?php if( $mask_url == 'on') { echo 'onclick="document.location.href = \''.$item_url.'\'; return false;"'; } ?> <?php echo $toolipsetup; ?> <?php echo esc_attr($qcopd_item_nofollow); ?> href="<?php echo $masked_url; ?>" data-subtitle="<?php echo esc_attr($list['qcopd_item_subtitle']); ?>" <?php echo (isset($list['qcopd_item_newtab']) && $list['qcopd_item_newtab'] == 1) ? 'target="_blank"' : ''; ?> >
						<?php endif; ?>
...
//adding extra variable in config
		...
		$qcopd_list_custom_css .= "
			...
			#qcopd-list-".$listId .'-'. get_the_ID().".style-9 ul{
				border-top: 5px solid ".$conf['item_bdr_color'].";
				padding-top: 10px !important;
			}
			#qcopd-list-".$listId .'-'. get_the_ID().".style-9 li a{
				color: ".$conf['list_txt_color'].";";
			...
			$qcopd_list_custom_css .= "}
			#qcopd-list-".$listId .'-'. get_the_ID().".style-9 li a:hover {
				color: ".$conf['list_txt_color_hov'].";

			}

			#qcopd-list-".$listId .'-'. get_the_ID().".style-9 .tooltip_tpl9:before {
				color: ".$conf['list_txt_color_hov'].";";
			...
			$qcopd_list_custom_css .= ".qcopd-list-wrapper .style-9 .featured-section {
        ...
        wp_add_inline_style( 'sld-css-style-9', $qcopd_list_custom_css );
		?>
		...
		<?php if( !empty($lists) && (count($lists) > -1) ){ ?>
			<div id="qcopd-list-<?php echo $listId .'-'. get_the_ID(); ?>" class="qc-grid-item qcopd-list-column opd-column-<?php echo $column; echo " " . $style;?> <?php echo "opd-list-id-" . get_the_ID(); ?>">
				...
				<div class="opd-list-style-9 qcopd-single-list">
					...
					<ul class="tooltip_tpl9-tpl" id="jp-list-<?php echo $SLD_QCOPD_DIRectory_instance_count.'-'.get_the_ID(); ?><?php echo (isset($cattabid)&&$cattabid!=''?'-'.$cattabid:''); ?>" <?php echo ($removetop=='yes'?'style="border-top:unset !important;"':''); ?>>
						...
						<li id="item-<?php echo get_the_ID() ."-". $count; ?>" style="<?php echo ( isset($list['list_item_bg_color']) && !empty($list['list_item_bg_color']) ) ? 'background:'. esc_attr($list['list_item_bg_color']) : ''; ?>">
							...
							<?php else: ?>
								...
								<a  title="<?php echo esc_attr($list['qcopd_item_title']); ?>"  <?php if( $mask_url == 'on') { echo 'onclick="document.location.href = \''.$item_url.'\'; return false;"'; } ?> <?php echo esc_attr($qcopd_item_nofollow); ?> href="<?php echo $masked_url; ?>"  data-subtitle="<?php echo esc_attr( $subtitle ); ?>"
								<?php echo (isset($list['qcopd_item_newtab']) && $list['qcopd_item_newtab'] == 1) ? 'target="_blank"' : ''; ?> data-itemid="<?php echo get_the_ID(); ?>" <?php echo $toolipsetup; ?> data-itemurl="<?php echo $list['qcopd_item_link']; ?>" data-itemsid="<?php echo $list['qcopd_timelaps']; ?>" data-tag="<?php echo (isset($list['qcopd_tags'])?$list['qcopd_tags']:'' ); ?>" <?php echo $popContent; ?>>
