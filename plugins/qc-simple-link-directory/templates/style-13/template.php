...
<?php wp_enqueue_style('sld-css-style-13', SLD_OCOPD_TPL_URL . "/$template_code/template.css" ); ?>
...
if ( $list_query->have_posts() )
{
	...
	echo '<div class="qcld-main-container-style-13"><div class="qcopd-list-wrapper '. $subtitle_present_class . ' qc-full-wrapper">';
	?>
	<?php
	//if(sld_get_option('sld_enable_filtering_left')=='on' || $enable_left_filter=='true') {
    if( ( sld_get_option('sld_enable_filtering_left')=='on' && $enable_left_filter !='false'  ) || $enable_left_filter=='true') {
...
<?php
	...
	if(is_user_logged_in() and $sldfavorite=='on'){
        ...
        wp_add_inline_style( 'sld-css-style-13', $bookmark_list_custom_css );
...
?>
		...
		<div id="bookmark_list" class="qc-grid-item qcopd-list-column opd-column-<?php echo 1; echo " " . $style; echo " " . $new_class;?> <?php echo "opd-list-id-" . get_the_ID(); ?>">
			...
			if(!empty($user_meta_data)){
				foreach($user_meta_data[0] as $postid=>$metaids){
					...
					?>
					<li id="sld_bookmark_li_<?php echo $b; ?>" class="ilist-25" style="<?php echo ( isset($list['list_item_bg_color']) && !empty($list['list_item_bg_color']) ) ? 'background:'. esc_attr($list['list_item_bg_color']) : ''; ?>">
						...
						<?php else: ?>
							<a title="<?php echo esc_attr($list['qcopd_item_title']); ?>" <?php if( $mask_url == 'on') { echo 'onclick="document.location.href = \''.$item_url.'\'; return false;"'; } ?>  <?php echo esc_attr($qcopd_item_nofollow); ?> href="<?php echo $masked_url; ?>" <?php echo (isset($list['qcopd_item_newtab']) && $list['qcopd_item_newtab'] == 1) ? 'target="_blank"' : ''; ?>  >
						<?php endif; ?>
...
if($bookmark_list == "favorite" ){
	...
	while ( $list_query->have_posts() )
	{
		...
		$qcopd_list_custom_css .= "
			...
			#qcopd-list-".$listId .'-'. get_the_ID()." ul li .item-title-text h3{
			  color: ".$conf['list_txt_color'].";";
				if($conf['item_title_font_size']!=''):
			...
			#qcopd-list-".$listId .'-'. get_the_ID()." ul li .item-title-text h3:hover{
			  color: ".$conf['list_txt_color_hov'].";
			}

			#qcopd-list-".$listId .'-'. get_the_ID()." ul li .item-desc-text p{
			  color: ".$conf['list_subtxt_color'].";";
				if($conf['item_subtitle_font_size']!=''): 
			...
			#qcopd-list-".$listId .'-'. get_the_ID()." ul li .item-desc-text:hover p{
			  color: ".$conf['list_subtxt_color_hov'].";
			}";
			...
			if( isset($conf['list_bg_featured_color']) && !empty(@$conf['list_bg_featured_color'])){ 
				$qcopd_list_custom_css .= ".qcopd-list-wrapper .style-13 .featured-section {
        ...
        wp_add_inline_style( 'sld-css-style-13', $qcopd_list_custom_css );
		?>
				...
				<div class="opd-list-style-12 sld-container title_style_sld qcopd-single-list" style="<?php echo ($listId==1)?'margin-top:10px':''; ?>">
					...
					<ul class="tooltip_tpl12-tpl sld-list" id="jp-list-<?php echo get_the_ID(); ?><?php echo (isset($cattabid)&&$cattabid!=''?'-'.$cattabid:''); ?>">
						...
						<li id="item-<?php echo get_the_ID() ."-". $count; ?>" class="ilist-25">
							...
							<div class="col-<?php echo $column; ?>">
									<div class="ilist-item-main" style="<?php echo ( isset($list['list_item_bg_color']) && !empty($list['list_item_bg_color']) ) ? 'background:'. esc_attr($list['list_item_bg_color']) : ''; ?>">


										<?php if(sld_is_audio_url($list['qcopd_item_link'])): ?>
											<a data-itemurl="<?php echo $list['qcopd_item_link']; ?>" title="<?php echo esc_attr($list['qcopd_item_title']); ?>"  <?php echo esc_attr($qcopd_item_nofollow); ?>  data-tag="<?php echo (isset($list['qcopd_tags'])?$list['qcopd_tags']:'' ); ?>" data-subtitle="<?php echo esc_attr( $subtitle ); ?>" >
										...
										<?php else: ?>
											<a data-itemurl="<?php echo $list['qcopd_item_link']; ?>" title="<?php echo esc_attr($list['qcopd_item_title']); ?>" <?php if( $mask_url == 'on') { echo 'onclick="document.location.href = \''.$item_url.'\'; return false;"'; } ?>  <?php echo esc_attr($qcopd_item_nofollow); ?> href="<?php echo $masked_url; ?>" <?php echo (isset($list['qcopd_item_newtab']) && $list['qcopd_item_newtab'] == 1) ? 'target="_blank"' : ''; ?> data-tag="<?php echo (isset($list['qcopd_tags'])?$list['qcopd_tags']:'' ); ?>" data-subtitle="<?php echo esc_attr( $subtitle ); ?>" <?php echo $popContent; ?>>
										<?php endif; ?>
								...
								<a title="<?php echo esc_attr($list['qcopd_item_title']); ?>" <?php echo $videoPopContent; ?> href="<?php echo $list['qcopd_item_link']; ?>" <?php if($video_click!='popup'){ echo (isset($list['qcopd_item_newtab']) && $list['qcopd_item_newtab'] == 1) ? 'target="_blank"' : ''; echo 'data-itemurl="'.$list['qcopd_item_link'].'"'; } ?>  data-mfp-src="#sldvideo-<?php echo $count; ?>" data-itemid="<?php echo get_the_ID(); ?>" data-videourl="<?php echo $list['qcopd_item_link']; ?>" data-itemsid="<?php echo $list['qcopd_timelaps']; ?>" data-tag="<?php echo (isset($list['qcopd_tags'])?$list['qcopd_tags']:'' ); ?>" >
							...
							<?php elseif(sld_is_youtube_video($extra_video_url)): ?>
								...
								<a title="<?php echo esc_attr($list['qcopd_item_title']); ?>" <?php echo $videoPopContent; ?> href="<?php echo esc_url($extra_video_url); ?>" <?php if($video_click!='popup'){ echo (isset($list['qcopd_item_newtab']) && $list['qcopd_item_newtab'] == 1) ? 'target="_blank"' : ''; echo 'data-itemurl="'.esc_attr($extra_video_url).'"'; } ?> data-mfp-src="#sldvideo-<?php echo $count; ?>" data-itemid="<?php echo esc_attr(get_the_ID()); ?>"  data-videourl="<?php echo esc_url($extra_video_url); ?>" data-itemsid="<?php echo esc_attr($list['qcopd_timelaps']); ?>" data-tag="<?php echo (isset($list['qcopd_tags'])?esc_attr($list['qcopd_tags']):'' ); ?>" >
							...
							<?php elseif(sld_is_vimeo_video($extra_video_url)): ?>
								...
								<a title="<?php echo esc_attr($list['qcopd_item_title']); ?>" <?php echo $videoPopContent; ?> href="<?php echo esc_url($extra_video_url); ?>" <?php if($video_click!='popup'){ echo (isset($list['qcopd_item_newtab']) && $list['qcopd_item_newtab'] == 1) ? 'target="_blank"' : ''; echo 'data-itemurl="'.esc_url($extra_video_url).'"'; } ?> data-mfp-src="#sldvideo-<?php echo $count; ?>" data-itemid="<?php echo esc_attr(get_the_ID()); ?>" data-videourl="<?php echo esc_url($extra_video_url); ?>" data-itemsid="<?php echo esc_attr($list['qcopd_timelaps']); ?>" >
							...
							<?php else: ?>
								...
								<?php echo (isset($list['qcopd_item_newtab']) && $list['qcopd_item_newtab'] == 1) ? 'target="_blank"' : ''; ?> data-itemid="<?php echo get_the_ID(); ?>" data-itemurl="<?php echo $list['qcopd_item_link']; ?>" data-itemsid="<?php echo $list['qcopd_timelaps']; ?>" <?php echo $popContent; ?> data-tag="<?php echo (isset($list['qcopd_tags'])?$list['qcopd_tags']:'' ); ?>">
								
							<?php endif; ?>
													
													<h3 class="item-list-title"><?php echo trim($list['qcopd_item_title']); ?></h3></a>
