<?php wp_enqueue_style('sld-css-style-14', SLD_OCOPD_TPL_URL . "/$template_code/style.css" ); ?>
<?php wp_enqueue_style('sld-css-style-14-responsive', SLD_OCOPD_TPL_URL . "/$template_code/responsive.css" ); ?>
...
if ( $list_query->have_posts() )
{
	...
	echo '<div class="qcld-main-container-style-14"><div class="qcopd-list-wrapper qc-full-wrapper">';
	?>
	<?php
	//if(sld_get_option('sld_enable_filtering_left')=='on' || $enable_left_filter=='true') {
    if( ( sld_get_option('sld_enable_filtering_left')=='on' && $enable_left_filter !='false'  ) || $enable_left_filter=='true') {
		...
		<div id="bookmark_list" class="qc-grid-item qcopd-list-column opd-column-<?php echo 1; echo " " . $style; echo " " . $new_class;?> <?php echo "opd-list-id-" . get_the_ID(); ?>">
			...
			if(!empty($user_meta_data)){
				foreach($user_meta_data[0] as $postid=>$metaids){
					...
					<li id="sld_bookmark_li_<?php echo $b; ?>" class="sld-26" style="<?php echo ( isset($list['list_item_bg_color']) && !empty($list['list_item_bg_color']) ) ? 'background:'. esc_attr($list['list_item_bg_color']) : ''; ?>">
						<?php
                                	...
                                	<div class="sld-main-content-area bg-color-0<?php echo (($b%5)+1); ?>">
										...
										<div class="style-14-upvote-section">
											...
											<?php elseif(sld_is_mail_url($list['qcopd_item_link'])): ?>
												<a href="<?php echo esc_attr($list['qcopd_item_link']); ?>" ><?php echo $visit_page; ?></a>
											<?php else: ?>
                                            <a <?php if( $mask_url == 'on') { echo 'onclick="document.location.href = \''.$item_url.'\'; return false;"'; } ?>  <?php echo esc_attr($qcopd_item_nofollow); ?> href="<?php echo $masked_url; ?>" <?php echo (isset($list['qcopd_item_newtab']) && $list['qcopd_item_newtab'] == 1) ? 'target="_blank"' : ''; ?> ><?php echo $visit_page; ?></a>
                                            <?php endif; ?>
	...
	while ( $list_query->have_posts() )
	{
		...
		$qcopd_list_custom_css .= "
			...
			if( isset($conf['list_bg_featured_color']) && !empty(@$conf['list_bg_featured_color'])){
				$qcopd_list_custom_css .= ".qcopd-list-wrapper .style-14 .featured-section {
        ...
        wp_add_inline_style( 'sld-css-style-14', $qcopd_list_custom_css );
				...
				<div class="opd-list-style-12 sld-container title_style_sld qcopd-single-list" style="<?php echo ($listId==1)?'margin-top:10px':''; ?>">
					...
					<div class="main-title">
							...
							<?php if($hide_list_title != 'true'){ ?>
	                            <h2 <?php echo (isset($conf['list_title_color'])&&$conf['list_title_color']!=''?'style="color:'.$conf['list_title_color'].';"':''); ?>><?php echo esc_html(get_the_title()); ?>
									<?php
										if($item_count == 'on'){
					...
					<ul class="tooltip_tpl12-tpl sld-list" id="jp-list-<?php echo $SLD_QCOPD_DIRectory_instance_count.'-'.get_the_ID(); ?><?php echo (isset($cattabid)&&$cattabid!=''?'-'.$cattabid:''); ?>">
						...
						<li id="item-<?php echo get_the_ID() ."-". $count; ?>" class="sld-26" style="<?php echo ( isset($list['list_item_bg_color']) && !empty($list['list_item_bg_color']) ) ? 'background:'. esc_attr($list['list_item_bg_color']) : ''; ?>">
							<?php
								$item_url = $list['qcopd_item_link'];
								$masked_url = $list['qcopd_item_link'];
	                            	...
	                            	<div class="column-grid<?php echo $column; ?>">
										...
										<?php } ?>
											...
											<?php elseif(sld_is_mail_url($list['qcopd_item_link'])): ?>
												...
												<?php echo esc_attr($list['qcopd_item_link']); ?></a>
											<?php else: ?>
