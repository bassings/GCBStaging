...
<?php wp_enqueue_style('sld-css-style-12', SLD_OCOPD_TPL_URL . "/$template_code/template.css" ); ?>
...
if ( $list_query->have_posts() ) 
{
	...
	echo '<div class="qcld-main-container-style-12"><div class="qcopd-list-wrapper '. $subtitle_present_class . ' qc-full-wrapper">';
	?>
	<?php
	//if(sld_get_option('sld_enable_filtering_left')=='on' || $enable_left_filter=='true') {
    if( ( sld_get_option('sld_enable_filtering_left')=='on' && $enable_left_filter !='false'  ) || $enable_left_filter=='true') {
	...
	if(is_user_logged_in() and $sldfavorite=='on'){
		...
		$bookmark_list_custom_css .= "
			...
			#bookmark_list.style-12 ul.sld-list li{";
				...
				$bookmark_list_custom_css .= "border-bottom: 1px solid ".$conf['item_bdr_color'].";";
				endif;
			$bookmark_list_custom_css .= "}
			...
			#bookmark_list.style-12 li{
			    ...
			    if( $conf['item_bdr_color'] != "" ) : 
				$bookmark_list_custom_css .= "border-color: -moz-use-text-color -moz-use-text-color ".$conf['item_bdr_color']." !important;";
				endif;
			$bookmark_list_custom_css .= "}
			...
			#bookmark_list.style-12 li:hover {
				...
				if( $conf['item_bdr_color'] != "" ) :
				$bookmark_list_custom_css .= "border-color: -moz-use-text-color -moz-use-text-color ".$conf['list_bg_color_hov']." !important;";
				endif; 
			$bookmark_list_custom_css .= "}
			...
			#bookmark_list.style-12 li:hover .upvote-section .upvote-btn, #qcopd-list-".$listId .'-'. get_the_ID().".style-12 li:hover .upvote-section .upvote-count{
			  color: ".$conf['list_subtxt_color_hov'].";
			}";

        wp_add_inline_style( 'sld-css-style-12', $bookmark_list_custom_css );
			...
			if(!empty($user_meta_data)){
				foreach($user_meta_data[0] as $postid=>$metaids){
					...
					<li id="sld_bookmark_li_<?php echo $b; ?>" style="<?php echo ( isset($list['list_item_bg_color']) && !empty($list['list_item_bg_color']) ) ? 'background:'. esc_attr($list['list_item_bg_color']) : ''; ?>">
						...
						<?php else: ?>
						<a <?php if( $mask_url == 'on') { echo 'onclick="document.location.href = \''.$item_url.'\'; return false;"'; } ?> <?php echo $toolipsetup; ?>  <?php echo esc_attr($qcopd_item_nofollow); ?> href="<?php echo $masked_url; ?>" <?php echo (isset($list['qcopd_item_newtab']) && $list['qcopd_item_newtab'] == 1) ? 'target="_blank"' : ''; ?> >
						<?php endif; ?>
				...
				<!-- navigation panel -->
				<div id="jp-holder-<?php echo $SLD_QCOPD_DIRectory_instance_count.'-'.get_the_ID(); ?>" class="sldp-holder"></div>
...
if($bookmark_list == "favorite" ){
	...
	while ( $list_query->have_posts() ) 
	{
		...
		$qcopd_list_custom_css .= "
			#qcopd-list-".$listId .'-'. get_the_ID().".style-12 h2{
				background: ".$conf['list_border_color'].";
			}
			
			#qcopd-list-".$listId .'-'. get_the_ID().".style-12 ul.sld-list li{";
				if( $conf['item_bdr_color'] != "" ) : 
				$qcopd_list_custom_css .= "border-bottom: 1px solid ".$conf['item_bdr_color'].";";
				endif; 
			$qcopd_list_custom_css .= "}

			#qcopd-list-".$listId .'-'. get_the_ID().".style-12 li{
			    background-color: ".$conf['list_bg_color'].";";
			    if( $conf['item_bdr_color'] != "" ) :
				$qcopd_list_custom_css .= "border-color: -moz-use-text-color -moz-use-text-color ".$conf['item_bdr_color']." !important;";
				endif;
			$qcopd_list_custom_css .= "}

			#qcopd-list-".$listId .'-'. get_the_ID().".style-12 li:hover {
			    background-color: ".$conf['list_bg_color_hov'].";
				color: ".$conf['list_txt_color_hov'].";";
				if( $conf['item_bdr_color'] != "" ) :
				$qcopd_list_custom_css .= "border-color: -moz-use-text-color -moz-use-text-color ".$conf['list_bg_color_hov']." !important;";
				endif;
			$qcopd_list_custom_css .= "}

			#qcopd-list-".$listId .'-'. get_the_ID().".style-12 li a{
			  color: ".$conf['list_txt_color'].";";
			  	if($conf['item_title_font_size']!=''):
			...
			#qcopd-list-".$listId .'-'. get_the_ID().".style-12 .tooltip_tpl8:before {
			  color: ".$conf['list_subtxt_color'].";";
			  ...
			  $qcopd_list_custom_css .= "background-color: ".$conf['list_subtxt_color_hov'].";
			  border: 1px solid;
			  border-color: ".$conf['list_subtxt_color_hov'].";
			}

			#qcopd-list-".$listId .'-'. get_the_ID().".style-12 .tooltip_tpl8:after {
			  border-bottom: 6px solid ".$conf['list_subtxt_color_hov'].";
			}

			#qcopd-list-".$listId .'-'. get_the_ID().".style-12 li:hover a{
			  color: ".$conf['list_txt_color_hov'].";
			}

			#qcopd-list-".$listId .'-'. get_the_ID().".style-12 li .upvote-section .upvote-btn, #qcopd-list-".$listId .'-'. get_the_ID().".style-12 .ca-menu li .upvote-section .upvote-count{
			  color: ".$conf['list_subtxt_color'].";
			}		

			#qcopd-list-".$listId .'-'. get_the_ID().".style-12 li:hover .upvote-section .upvote-btn, #qcopd-list-".$listId .'-'. get_the_ID().".style-12 li:hover .upvote-section .upvote-count{
			  color: ".$conf['list_subtxt_color_hov'].";
			}
			
			#item-".$listId .'-'. get_the_ID()."-add-block .advertise-block.tpl-default{
				border: 1px solid #dedede;
				border-bottom: 2px solid ".$conf['item_bdr_color'].";
			}
			...
			if( isset($conf['list_bg_featured_color']) && !empty(@$conf['list_bg_featured_color'])){
				$qcopd_list_custom_css .= ".qcopd-list-wrapper .style-12 .featured-section {
        ...
        wp_add_inline_style( 'sld-css-style-12', $qcopd_list_custom_css );
		?>
		...
		<?php if( !empty($lists) && (count($lists) > -1) ){ ?>
			<div id="qcopd-list-<?php echo $listId .'-'. get_the_ID(); ?>" class="qc-grid-item qcopd-list-column opd-column-<?php echo $column; echo " " . $style;?> <?php echo "opd-list-id-" . get_the_ID(); ?>">
				...
				<div class="opd-list-style-12 sld-container qcopd-single-list">
					...
					<?php } ?>
					<ul class="tooltip_tpl12-tpl sld-list" id="jp-list-<?php echo $SLD_QCOPD_DIRectory_instance_count.'-'.get_the_ID(); ?><?php echo (isset($cattabid)&&$cattabid!=''?'-'.$cattabid:''); ?>">
						<?php $count = 1; 
							if( $item_orderby == 'upvotes' )
							{
								if( $item_order == 'ASC' ){
						...
						<li id="item-<?php echo get_the_ID() ."-". $count; ?>" style="<?php echo ( isset($list['list_item_bg_color']) && !empty($list['list_item_bg_color']) ) ? 'background:'. esc_attr($list['list_item_bg_color']) : ''; ?>">
							...
							<?php if( $display_username == 'true' ){ ?>
								<span class="qcopd-list-item-author" >
									<?php
										...
										if( $userinfo ){
											echo esc_html( 'by '.$userinfo->display_name );
										}else{
											echo esc_html( 'by '. get_the_author_meta('display_name') );
										}
									?>
								</span>
							<?php } ?>
							...
							<?php 
								...
								if($main_click=='popup'){
									$masked_url = '#';
									if(isset($list['qcopd_item_subtitle']) and $list['qcopd_item_subtitle']!=''){
										$popContent = 'class="open-mpf-sld-more2 sld_load_more2 tooltip-element tooltip_tpl8 fade" data-post-id="'.get_the_ID().'" data-title="'.trim($list['qcopd_item_subtitle']).'" data-item-title="'.trim($list['qcopd_item_title']).'" data-item-link="'.$list['qcopd_item_link'].'" data-mfp-src="#sldinfo-'.get_the_ID() ."-". $count. "-". esc_attr($list['qcopd_timelaps']).'"';
								...
								if( $video_click=='popup' ){
									$videoPopContent = ' class="open-mpf-sld-video sld_load_video tooltip-element tooltip_tpl8 fade" ';
								}
							...
							<?php if(sld_is_youtube_video($item_url)): ?>
								...
								<a title="<?php echo esc_attr($list['qcopd_item_title']); ?>" <?php echo $videoPopContent; ?> href="<?php echo $list['qcopd_item_link']; ?>" <?php if($video_click!='popup'){ echo (isset($list['qcopd_item_newtab']) && $list['qcopd_item_newtab'] == 1) ? 'target="_blank"' : ''; echo 'data-itemurl="'.$list['qcopd_item_link'].'"'; } ?>  data-mfp-src="#sldvideo-<?php echo $count; ?>" <?php echo $toolipsetup; ?> data-itemid="<?php echo get_the_ID(); ?>" data-videourl="<?php echo $list['qcopd_item_link']; ?>" data-itemsid="<?php echo $list['qcopd_timelaps']; ?>" data-tag="<?php echo (isset($list['qcopd_tags'])?$list['qcopd_tags']:'' ); ?>" data-subtitle="<?php echo esc_attr( $subtitle ); ?>" >
							...
							<?php else: ?>
								...
								<a title="<?php echo esc_attr($list['qcopd_item_title']); ?>" <?php if( $mask_url == 'on') { echo 'onclick="document.location.href = \''.$item_url.'\'; return false;"'; } ?> <?php echo esc_attr($qcopd_item_nofollow); ?> href="<?php echo $masked_url; ?>"
								<?php echo (isset($list['qcopd_item_newtab']) && $list['qcopd_item_newtab'] == 1) ? 'target="_blank"' : ''; ?> data-itemid="<?php echo get_the_ID(); ?>" <?php echo $toolipsetup; ?> data-itemurl="<?php echo $list['qcopd_item_link']; ?>" data-itemsid="<?php echo $list['qcopd_timelaps']; ?>" data-tag="<?php echo (isset($list['qcopd_tags'])?$list['qcopd_tags']:'' ); ?>" data-subtitle="<?php echo esc_attr( $subtitle ); ?>" <?php echo $popContent; ?>>
				...
				<!-- Add Block -->
				<div  id="item-<?php echo $listId .'-'. get_the_ID(); ?>-add-block">
