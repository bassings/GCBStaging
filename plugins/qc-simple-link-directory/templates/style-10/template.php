...
<?php wp_enqueue_style('sld-css-style-10', SLD_OCOPD_TPL_URL . "/$template_code/template.css" ); ?>
...
if ( $list_query->have_posts() )
{
	...
	echo '<div class="qcld-main-container-style-10"><div class="qcopd-list-wrapper '. $subtitle_present_class . ' qc-full-wrapper">';
	?>
	<?php
	//if(sld_get_option('sld_enable_filtering_left')=='on' || $enable_left_filter=='true') {
    if( ( sld_get_option('sld_enable_filtering_left')=='on' && $enable_left_filter !='false'  ) || $enable_left_filter=='true') {
...
//If FILTER TYPE is NORMAL
	...
	if(is_user_logged_in() and $sldfavorite=='on'){
		...
		$bookmark_list_custom_css .= "
			...
			#bookmark_list.style-10 ul li .upvote-section .upvote-btn, #qcopd-list-".$listId .'-'. get_the_ID().".style-10 ul li .upvote-section .upvote-count{
			  color: ".$conf['list_subtxt_color'].";
			}

			#bookmark_list.style-10 ul li:hover .upvote-section .upvote-btn, #qcopd-list-".$listId .'-'. get_the_ID().".style-10 ul li:hover .upvote-section .upvote-count{
			  color: ".$conf['list_subtxt_color_hov'].";
			}";

        wp_add_inline_style( 'sld-css-style-10', $bookmark_list_custom_css );
...
?>
		...
		<div id="bookmark_list" class="qcopd-list-column <?php echo $style; echo " " . $new_class; ?>">
				...
				<section class="list-section-seven">
			        <div class="container-style-10">
			            <div class="row-style-10">
			                <ul class="portfolio-listing ca-menu column<?php echo $column; ?>" id="sld_bookmark_ul">
				...
				foreach($user_meta_data[0] as $postid=>$metaids){
					...
					if(!empty($metaids)){
						foreach($metaids as $metaid){
							...
							$results = $wpdb->get_results( $wpdb->prepare( "SELECT * FROM $wpdb->postmeta WHERE post_id = $postid AND meta_key = 'qcopd_list_item01'", $postid ) );
							if(!empty($results)){
					...
					?>
                    <li id="sld_bookmark_li_<?php echo $b; ?>"  class="list-style-seven listy-style-seven-01 sld-style10-column<?php echo $column; ?>"  style="<?php echo ( isset($list['list_item_bg_color']) && !empty($list['list_item_bg_color']) ) ? 'background:'. esc_attr($list['list_item_bg_color']) : ''; ?>">
						<?php
						...
						<?php else: ?>
						<a <?php if( $mask_url == 'on') { echo 'onclick="document.location.href = \''.$item_url.'\'; return false;"'; } ?>  <?php echo esc_attr($qcopd_item_nofollow); ?> href="<?php echo $masked_url; ?>" <?php echo (isset($list['qcopd_item_newtab']) && $list['qcopd_item_newtab'] == 1) ? 'target="_blank"' : ''; ?> >
						<?php endif; ?>
...
if($bookmark_list == "favorite" ){
	...
	while ( $list_query->have_posts() )
	{
		...
		$qcopd_list_custom_css .= "
			...
			#qcopd-list-".$listId .'-'. get_the_ID().".style-10 ul li:hover .upvote-section .upvote-btn, #qcopd-list-<?php echo $listId .'-'. get_the_ID(); ?>.style-10 ul li:hover .upvote-section .upvote-count{
			  color: ".$conf['list_subtxt_color_hov'].";
			}
			...
			$qcopd_list_custom_css .= ".qcopd-list-wrapper .style-10 .featured-section {
			...
			.row-style-10 .effect-style-seven {
			    background: transparent;
			}";

			}

        wp_add_inline_style( 'sld-css-style-10', $qcopd_list_custom_css );
		?>
			...
			<div id="qcopd-list-<?php echo $listId .'-'. get_the_ID(); ?>" class="qcopd-list-column <?php echo $style; ?>">
				...
				<div class="qcopd-single-list-1">
					...
					<section class="list-section-seven">
				        <div class="container-style-10">
				            <div class="row-style-10">
				                <ul class="portfolio-listing ca-menu column<?php echo $column; ?>">
						<?php $count = 1; 
							if( $item_orderby == 'upvotes' )
							{
								if( $item_order == 'ASC' ){
	                    ...
	                    <li id="item-<?php echo get_the_ID() ."-". $count; ?>"  class="list-style-seven listy-style-seven-01 sld-style10-column<?php echo $column; ?>">
							...
							<?php if(sld_is_youtube_video($item_url)): ?>
								...
								<a  title="<?php echo esc_attr($list['qcopd_item_title']); ?>"  <?php echo $videoPopContent; ?> href="<?php echo esc_url($list['qcopd_item_link']); ?>" <?php if($video_click!='popup'){ echo (isset($list['qcopd_item_newtab']) && $list['qcopd_item_newtab'] == 1) ? 'target="_blank"' : ''; echo 'data-itemurl="'.$list['qcopd_item_link'].'"'; } ?> data-mfp-src="#sldvideo-<?php echo $count; ?>" data-itemid="<?php echo get_the_ID(); ?>" data-videourl="<?php echo esc_url($list['qcopd_item_link']); ?>" data-itemsid="<?php echo $list['qcopd_timelaps']; ?>" data-tag="<?php echo (isset($list['qcopd_tags'])?$list['qcopd_tags']:'' ); ?>" data-subtitle="<?php echo esc_attr( $subtitle ); ?>" >
							...
							<?php endif; ?>
								...
								?>
									...
									<?php elseif( $showFavicon == 1 && $faviconFetchable == true ) : ?>
										<span class="ca-icon list-img-1" style="background-image: url(<?php echo $faviconImgUrl; ?>);">
										&nbsp;
										</span>
	                                ...
	                                <div class="effect-style-seven" style="<?php echo ( isset($list['list_item_bg_color']) && !empty($list['list_item_bg_color']) ) ? 'background:'. esc_attr($list['list_item_bg_color']) : ''; ?>">
	                                    <h3>
	                                    	<?php
												echo trim($list['qcopd_item_title']);
											?>
