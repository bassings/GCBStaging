...
<?php wp_enqueue_style('sld-css-style-7', SLD_OCOPD_TPL_URL . "/$template_code/template.css" ); ?>
...
if ( $list_query->have_posts() )
{
	...
	echo '<div class="qcld-main-container-style-7"><div class="qcopd-list-wrapper '. $subtitle_present_class . ' qc-full-wrapper">';
	?>
	<?php
	//if(sld_get_option('sld_enable_filtering_left')=='on' || $enable_left_filter=='true') {
    if( ( sld_get_option('sld_enable_filtering_left')=='on' && $enable_left_filter !='false'  ) || $enable_left_filter=='true') {
					...
					<li id="sld_bookmark_li_<?php echo $b; ?>" style="<?php echo ( isset($list['list_item_bg_color']) && !empty($list['list_item_bg_color']) ) ? 'background:'. esc_attr($list['list_item_bg_color']) : ''; ?>">
						...
						<?php endif; ?>
							...
							<?php
								...
								$showFavicon = (isset($list['qcopd_use_favicon']) && trim($list['qcopd_use_favicon']) != "") ? esc_attr($list['qcopd_use_favicon']) : "";
...
//adding extra variable in config
		...
		$qcopd_list_custom_css .= "
			...
			#qcopd-list-".$listId .'-'. get_the_ID().".style-7 .ca-menu li:hover .ca-main {
			  color: ".$conf['list_txt_color_hov'].";
			}

			#qcopd-list-".$listId .'-'. get_the_ID().".style-7 .ca-menu li .ca-sub {
			...
			#qcopd-list-".$listId .'-'. get_the_ID().".style-7 .ca-menu li:hover .ca-sub {
			  color: ".$conf['list_subtxt_color_hov'].";
			}

			#qcopd-list-".$listId .'-'. get_the_ID().".style-7 .ca-menu li .upvote-section .upvote-btn, #qcopd-list-".$listId .'-'. get_the_ID().".style-7 .ca-menu li .upvote-section .upvote-count{
			  color:".$conf['list_subtxt_color'].";
			}

			#qcopd-list-".$listId .'-'. get_the_ID().".style-7 .ca-menu li:hover .upvote-section .upvote-btn, #qcopd-list-".$listId .'-'. get_the_ID().".style-7 .ca-menu li:hover .upvote-section .upvote-count{
			  color: ".$conf['list_subtxt_color_hov'].";
			}
			...
			if( isset($conf['list_bg_featured_color']) && !empty(@$conf['list_bg_featured_color'])){ 
				$qcopd_list_custom_css .= ".qcopd-list-wrapper .style-7 .featured-section {
        ...
        wp_add_inline_style( 'sld-css-style-7', $qcopd_list_custom_css );
		?>
				...
				<div class="qcopd-single-list">
					...
					<ul class="ca-menu column<?php echo $column; ?>" id="jp-list-<?php echo $SLD_QCOPD_DIRectory_instance_count.'-'.get_the_ID(); ?><?php echo (isset($cattabid)&&$cattabid!=''?'-'.$cattabid:''); ?>">
						...
						<?php if( class_exists('QCOPD\QCOPD_Review_Rating') && function_exists('qcopdr_ot_get_option')  && qcopdr_ot_get_option('qcopdr_enable_reviews') != 'off' && ( isset($review) && $review=='true' ) ){ ?>
							...
							?>
							<div data-list-id="<?php echo get_the_ID(); ?>" data-item-id="<?php echo $list['qcopd_timelaps']; ?>" data-mfp-src="#sld-item-average-rating-form-<?php echo $count.'-'.get_the_ID().'-'.$list['qcopd_timelaps']; ?>" class="sld-rating-inverse sld-item-review-opener sld-item-review-opener-active">
