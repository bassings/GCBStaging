<?php wp_enqueue_style('sld-css-style-4', SLD_OCOPD_TPL_URL . "/$template_code/template.css" ); ?>
...
if ( $list_query->have_posts() )
{
	...
	echo '<div class="qcld-main-container-style-4"><div class="qcopd-list-wrapper style-4-grid-wrapper '. $subtitle_present_class . ' qc-full-wrapper">';
	?>
	<?php
	//if(sld_get_option('sld_enable_filtering_left')=='on' || $enable_left_filter=='true') {
    if( ( sld_get_option('sld_enable_filtering_left')=='on' && $enable_left_filter !='false'  ) || $enable_left_filter=='true') {
...
//If FILTER TYPE is NORMAL
	...
	$actual_pagina = ( isset($actual_pagination) && ( $actual_pagination == 'true' ) ) ? 's' : '';

	echo '<div id="opd-list-holder" class="qc-grid'.$actual_pagina.' qc-style4-grid qcopd-list-hoder '.$rtlClass.'">';
	global $wpdb;
	if(is_user_logged_in() and $sldfavorite=='on'){
		...
		$bookmark_list_custom_css .= "#bookmark_list.style-4 .ca-menu li {
        ...
        wp_add_inline_style( 'sld-css-style-4', $bookmark_list_custom_css );
		...
		<div id="bookmark_list" class="<?php echo $style;  echo " " . $new_class; ?>">
			...
			if(!empty($user_meta_data)){
				foreach($user_meta_data[0] as $postid=>$metaids){
					...
					?>
					<li id="sld_bookmark_li_<?php echo $b; ?>" class="style-4-<?php echo $column; ?>" style="<?php echo ( isset($list['list_item_bg_color']) && !empty($list['list_item_bg_color']) ) ? 'background:'. esc_attr($list['list_item_bg_color']) : ''; ?>">
						<?php
...
//adding extra variable in config
		...
		$qcopd_list_custom_css .= "
			#qcopd-list-".$listId .'-'. get_the_ID().".style-4 .ca-menu li {
			    /*background-color: ".$conf['list_bg_color'].";
				box-shadow: 1px 1px 2px  ".$conf['item_bdr_color'].";*/
			}

			#qcopd-list-".$listId .'-'. get_the_ID().".style-4 .ca-menu li:hover {
			    /*background-color: ".$conf['list_bg_color_hov'].";
				color: ".$conf['list_txt_color_hov'].";
				box-shadow: 1px 1px 2px ".$conf['item_bdr_color_hov'].";*/
			}

			#qcopd-list-".$listId .'-'. get_the_ID().".style-4 .ca-menu li .ca-main {
			  color: ".$conf['list_txt_color'].";";
			...
			#qcopd-list-".$listId .'-'. get_the_ID().".style-4 .ca-menu li:hover .ca-main {
			  color: ".$conf['list_txt_color_hov'].";
			}

			#qcopd-list-".$listId .'-'. get_the_ID().".style-4 .ca-menu li .ca-sub {
			  color: ".$conf['list_subtxt_color'].";";
			...
			#qcopd-list-".$listId .'-'. get_the_ID().".style-4 .ca-menu li:hover .ca-sub {
			  color: ".$conf['list_subtxt_color_hov'].";
			}

			#qcopd-list-".$listId .'-'. get_the_ID().".style-4 .ca-menu li .upvote-section .upvote-btn, #qcopd-list-".$listId .'-'. get_the_ID().".style-4 .ca-menu li .upvote-section .upvote-count{
			  color: ".$conf['list_subtxt_color'].";
			}

			#qcopd-list-".$listId .'-'. get_the_ID().".style-4 .ca-menu li:hover .upvote-section .upvote-btn, #qcopd-list-".$listId .'-'. get_the_ID().".style-4 .ca-menu li:hover .upvote-section .upvote-count{
			  color: ".$conf['list_subtxt_color_hov'].";
			}
			...
			if( isset($conf['list_bg_featured_color']) && !empty(@$conf['list_bg_featured_color'])){ 
				...
				$qcopd_list_custom_css .= ".qcopd-list-wrapper .style-4 .featured-section {
        ...
        wp_add_inline_style( 'sld-css-style-4', $qcopd_list_custom_css );
		?>
		...
		<?php if( !empty($lists) && (count($lists) > -1) ){ ?>
			<div id="qcopd-list-<?php echo $listId .'-'. get_the_ID(); ?>" class="<?php echo $style; ?>">
				...
				<div class="qcopd-single-list">
					...
					<ul class="ca-menu column<?php echo $column; ?>" id="jp-list-<?php echo $SLD_QCOPD_DIRectory_instance_count.'-'.get_the_ID(); ?><?php echo (isset($cattabid)&&$cattabid!=''?'-'.$cattabid:''); ?>">
						...
						?>
						<li id="item-<?php echo get_the_ID() ."-". $count; ?>" class="style-4-<?php echo $column; ?>" style="<?php echo ( isset($list['list_item_bg_color']) && !empty($list['list_item_bg_color']) ) ? 'background:'. esc_attr($list['list_item_bg_color']) : ''; ?>">
							...
							<?php else: ?>
								...
								<a title="<?php echo esc_attr($list['qcopd_item_title']); ?>" <?php if( $mask_url == 'on') { echo 'onclick="document.location.href = \''.$item_url.'\'; return false;"'; } ?> <?php echo esc_attr($qcopd_item_nofollow); ?> href="<?php echo esc_url($masked_url); ?>"
								<?php echo (isset($list['qcopd_item_newtab']) && $list['qcopd_item_newtab'] == 1) ? 'target="_blank"' : ''; ?> data-itemid="<?php echo get_the_ID(); ?>" data-itemurl="<?php echo esc_url($list['qcopd_item_link']); ?>" data-itemsid="<?php echo $list['qcopd_timelaps']; ?>" data-tag="<?php echo (isset($list['qcopd_tags'])?esc_html($list['qcopd_tags']):'' ); ?>" data-subtitle="<?php echo esc_attr( $subtitle ); ?>" <?php echo $popContent; ?>>
							...
							<?php endif; ?>
								...
								<p class="qcopd-item-link"><a <?php echo (isset($list['qcopd_item_nofollow']) && $list['qcopd_item_nofollow'] == 1) ? 'rel="nofollow"' : ''; ?> href="<?php echo $item_url; ?>" class="sld_internal_link" <?php echo (isset($list['qcopd_item_newtab']) && $list['qcopd_item_newtab'] == 1) ? 'target="_blank"' : ''; ?>><i class="fa fa-link" aria-hidden="true"></i><span class="sld-link-text"><?php echo $out = strlen($item_url) > 30 ? substr($item_url,0,30)."..." : $item_url; ?></span></a></p>
							...
							<?php if( $display_username == 'true' ){ ?>
								<span class="qcopd-list-item-author" >
									<?php
										...
										if( $userinfo ){
											echo $sld_lan_by.' '." <strong>".esc_html( $userinfo->nickname )."</strong>";
										}else{
											echo $sld_lan_by.' '." <strong>".esc_html( get_the_author_meta('nickname') )."</strong>";
										}
									?>
								</span>
							<?php } ?>
							...
							<div class="style-4-upvote-section">
								...
								<div class="bookmark-section bookmark-section-style-4">
									...
									<span class="open-mpf-sld-more sld_load_more" data-post-id="<?php echo get_the_ID(); ?>" data-item-title="<?php echo esc_attr(trim($list['qcopd_item_title'])); ?>" data-item-link="<?php echo esc_url($list['qcopd_item_link']); ?>" style="cursor:pointer" data-mfp-src="#sldinfo-<?php echo get_the_ID() ."-". $count ."-". esc_attr($list['qcopd_timelaps']); ?>">
			...
			<!-- Add Block -->
			<div class="qcopd-list-column style-4-add-block opd-column-<?php echo $column; ?> <?php echo "opd-list-id-" . get_the_ID(); ?>" id="item-<?php echo $listId .'-'. get_the_ID(); ?>-add-block">
