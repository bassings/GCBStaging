<?php wp_enqueue_style('sld-css-style-16', SLD_OCOPD_TPL_URL . "/$template_code/template.css" ); ?>

<?php if( $tooltip == true ) : ?>
<?php wp_enqueue_style('sld-css-style-16-tooltip', SLD_OCOPD_TPL_URL . "/$template_code/html5tooltips.css" ); ?>
<?php wp_enqueue_style('sld-css-style-16-tooltip-animate', SLD_OCOPD_TPL_URL . "/$template_code/html5tooltips.animation.css" ); ?>
<?php wp_enqueue_script('sld-style-16-html5tooltips', SLD_OCOPD_TPL_URL . "/$template_code/html5tooltips.js"); ?>
<?php endif; ?>
...
if ( $list_query->have_posts() )
{
    ...
    if( ( sld_get_option('sld_enable_filtering_left')=='on' && $enable_left_filter !='false'  ) || $enable_left_filter=='true') {
		...
		if($style=="style-16" && $infinityscroll==1){
	...
	echo '<div class="qcld-main-container-style-16"><div class="qcopd-list-wrapper qc-full-wrapper qcsld-new-style-16'. $subtitle_present_class . '">';
	?>
...
<?php
	...
	if(is_user_logged_in() and $sldfavorite=='on'){
        ...
        $bookmark_list_custom_css = "#bookmark_list.style-16 ul{
			...
			#bookmark_list.style-16 ul li a:hover{
			...
			#bookmark_list.style-16 .upvote-section .upvote-btn, #bookmark_list.style-16 .upvote-section .upvote-count {
				color: ".(sld_get_option('sld_bookmark_item_text_color')!=''?sld_get_option('sld_bookmark_item_text_color'):'').";
			}

			#bookmark_list.style-16 .upvote-section .upvote-btn:hover, #bookmark_list.style-16 li:hover .upvote-btn, #bookmark_list.style-16 li:hover .upvote-count{
				color: ".(sld_get_option('sld_bookmark_item_text_color_hover')!=''?sld_get_option('sld_bookmark_item_text_color_hover'):'').";
			}";

        wp_add_inline_style( 'sld-css-style-16', $bookmark_list_custom_css );
...
?>
					<li id="sld_bookmark_li_<?php echo $b; ?>" <?php echo esc_html($tooltip_content); ?>  style="<?php echo ( isset($list['list_item_bg_color']) && !empty($list['list_item_bg_color']) ) ? 'background:'. esc_attr($list['list_item_bg_color']) : ''; ?>">
						...
						<?php } ?>
							...
							<!-- upvote section -->
							<div class="bookmark-section bookmark-section-style-16">
...
if($bookmark_list == "favorite" ){
	...
	while ( $list_query->have_posts() )
	{
		...
		<?php if( $style == "style-16" ): 
			...
			$list_item_custom_css .= "body #list-item-".$listId .'-'. get_the_ID().".style-16 .qcopd-single-list h2:after{
				background: ".@$conf['list_border_color'].";
			}


			#list-item-".$listId .'-'. get_the_ID().".style-16 ul li{
				...
				$list_item_custom_css .= "border-bottom: 2px solid ".@$conf['item_bdr_color']." !important;";
				endif;
			$list_item_custom_css .= "}

			#list-item-".$listId .'-'. get_the_ID().".style-16 ul li:hover{
				background-color: ".@$conf['list_bg_color_hov'].";
				color: ".@$conf['list_txt_color_hov'].";";
				if( @$conf['item_bdr_color_hov'] != "" ) :
				$list_item_custom_css .= "border-bottom: 2px solid ".@$conf['item_bdr_color_hov']." !important;";
				endif;
			...
			#list-item-".$listId .'-'. get_the_ID().".style-16 .upvote-section .upvote-btn, #list-item-".$listId .'-'. get_the_ID().".style-16 .upvote-section .upvote-count {
				color: ".@$conf['list_txt_color'].";
			}

			#list-item-".$listId .'-'. get_the_ID().".style-16 .upvote-section .upvote-btn:hover, #list-item-".$listId .'-'. get_the_ID().".style-16 li:hover .upvote-btn, #list-item-".$listId .'-'. get_the_ID().".style-16 li:hover .upvote-count{
				color: ".@$conf['list_txt_color_hov'].";
			}
			...
			if( isset($conf['list_bg_featured_color']) && !empty(@$conf['list_bg_featured_color'])){
				$list_item_custom_css .= ".qcopd-list-wrapper .style-16 .featured-section {
			...
			wp_add_inline_style( 'sld-css-style-16', $list_item_custom_css );
				...
				<div class="qcopd-single-list">
					...
					<ul id="jp-list-<?php echo $SLD_QCOPD_DIRectory_instance_count.'-'.get_the_ID(); ?><?php echo (isset($cattabid)&&$cattabid!=''?'-'.$cattabid:''); ?>">
						<?php
							...
							//print_r($list);exit;
						?>
						<li id="item-<?php echo get_the_ID() ."-". $count; ?>" <?php echo ($tooltip_content); ?> style="<?php echo ( isset($list['list_item_bg_color']) && !empty($list['list_item_bg_color']) ) ? 'background:'. esc_attr($list['list_item_bg_color']) : ''; ?>" >
							...
							<?php
								...
								$popContent = '';
								$videoPopContent = '  class="qcsld-custom-class" ';
								if($main_click=='popup'){
									$masked_url = '#';
								...
								if( $video_click=='popup' ){
									$videoPopContent = ' class="open-mpf-sld-video sld_load_video qcsld-custom-class" ';
								}
							...
							<?php if(sld_is_youtube_video($item_url)): ?>
								...
								<a  title="<?php echo esc_attr($list['qcopd_item_title']); ?>" <?php echo $videoPopContent; ?> href="<?php echo esc_url($list['qcopd_item_link']); ?>" <?php if($video_click!='popup'){ echo (isset($list['qcopd_item_newtab']) && $list['qcopd_item_newtab'] == 1) ? 'target="_blank"' : ''; echo 'data-itemurl="'.esc_attr($list['qcopd_item_link']).'"'; } ?> data-mfp-src="#sldvideo-<?php echo $count; ?>" data-itemid="<?php echo esc_attr(get_the_ID()); ?>"  data-videourl="<?php echo esc_url($list['qcopd_item_link']); ?>" data-itemsid="<?php echo esc_attr($list['qcopd_timelaps']); ?>" data-tag="<?php echo (isset($list['qcopd_tags'])?esc_attr($list['qcopd_tags']):'' ); ?>" data-subtitle="<?php echo esc_attr( $subtitle ); ?>" >
							...
							<?php elseif(sld_is_mail_url($list['qcopd_item_link'])): ?>
								...
								<a class="qcsld-custom-class" title="<?php echo esc_attr($list['qcopd_item_title']); ?>" href="mailto:<?php echo esc_attr($list['qcopd_item_link']); ?>"  data-itemid="<?php echo esc_attr(get_the_ID()); ?>" data-videourl="<?php echo esc_url($extra_video_url); ?>" data-itemsid="<?php echo esc_attr($list['qcopd_timelaps']); ?>" data-subtitle="<?php echo esc_attr( $subtitle ); ?>" >
							...
							<?php else: ?>
								...
								<a class="qcsld-custom-class" title="<?php echo esc_attr($list['qcopd_item_title']); ?>" <?php if( $mask_url == 'on') { echo 'onclick="document.location.href = \''.$item_url.'\'; return false;"'; } ?> <?php echo esc_attr($qcopd_item_nofollow); ?> href="<?php echo esc_url($masked_url); ?>"
								<?php echo (isset($list['qcopd_item_newtab']) && $list['qcopd_item_newtab'] == 1) ? 'target="_blank"' : ''; ?> data-itemid="<?php echo esc_attr(get_the_ID()); ?>" data-itemurl="<?php echo esc_url($list['qcopd_item_link']); ?>" data-itemsid="<?php echo esc_attr($list['qcopd_timelaps']); ?>" data-tag="<?php echo (isset($list['qcopd_tags'])?esc_attr($list['qcopd_tags']):'' ); ?>" <?php echo $popContent; ?>  data-subtitle="<?php echo esc_attr( $subtitle ); ?>" >
						...
						<?php } ?>
							...
							<?php if( $upvote == 'on' ) : ?>
								...
								<!-- upvote section -->
								<div class="upvote-section upvote-section-style-16">
...
var template = '".$style."';";
	...
	jQuery(document).ready(function($){
		...
		$('.qcsld-new-style-16 .qc-grid-item ul').each(function() {
		  	var h = $(this).height();
		  	if( h > 280){
		  		//console.log( h );
...
if($style=='style-16' and $infinityscroll==1){
