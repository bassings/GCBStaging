<?php wp_enqueue_style('sld-css-style-1', SLD_OCOPD_TPL_URL . "/$template_code/template.css" ); ?>
...
if ( $list_query->have_posts() )
{
            ...
            <div class="filter-area" style="width: 100%;">
                ...
                <div class="filter-carousel">
					...
					<?php foreach ( $listItems as $item ) :
                        ...
                        <div class="item">
                            <a href="#" title="<?php echo esc_attr($item->post_title); ?>" class="filter-btn" data-filter="opd-list-id-<?php echo $item->ID; ?>"
	...
	echo '<div class="qcld-main-container-style-1"><div class="qcopd-list-wrapper '. $subtitle_present_class . 'qc-full-wrapper">';
	?>
	<?php
	//if(sld_get_option('sld_enable_filtering_left')=='on' || $enable_left_filter=='true') {
    if( ( sld_get_option('sld_enable_filtering_left')=='on' && $enable_left_filter !='false'  ) || $enable_left_filter=='true') {
...
?>
        ...
        // sld-css-style-1
        $bookmark_custom_css = "#bookmark_list.style-1 .ca-menu li {
			    border-left: 10px solid ".$conf['list_border_color'].";
			...
			if( isset($conf['item_title_font_size']) && $conf['item_title_font_size']!=''):
				$bookmark_custom_css .= "font-size: ".$conf['item_title_font_size']." !important;";
			endif;

			if( isset($conf['item_title_line_height']) && $conf['item_title_line_height']!=''):
				$bookmark_custom_css .= "line-height: ".$conf['item_title_line_height']." !important;";
			endif;
			$bookmark_custom_css .= "}";

			$bookmark_custom_css .= "#bookmark_list.style-1 .ca-menu li:hover .ca-main {
			  color: ".$conf['list_txt_color_hov'].";}";

			$bookmark_custom_css .= "#bookmark_list.style-1 .ca-menu li .ca-sub {
			  color: ".$conf['list_subtxt_color'].";";
				if( isset($conf['item_subtitle_font_size']) && $conf['item_subtitle_font_size']!=''):
				$bookmark_custom_css .= "font-size: ".$conf['item_subtitle_font_size']." !important;";
				endif;

				if( isset($conf['item_subtitle_line_height']) && $conf['item_subtitle_line_height']!=''):
				$bookmark_custom_css .= "line-height:".$conf['item_subtitle_line_height']."!important;";
				endif;
			$bookmark_custom_css .= "}";

			$bookmark_custom_css .= "#bookmark_list.style-1 .ca-menu li:hover .ca-sub {
        ...
        wp_add_inline_style( 'sld-css-style-1', $bookmark_custom_css );



		if($favorite_hide != "hide" ){ 
			...
			?>
		<div id="bookmark_list" class="qc-grid-item qcopd-list-column opd-column-<?php echo $column; echo " " . $style; echo " " . $new_class;?> <?php echo "opd-list-id-" . get_the_ID(); ?>">
			...
			<div class="qcopd-single-list">
				...
				<ul class="ca-menu" id="sld_bookmark_ul">
					...
					foreach($lists as $list){
						...
						if($exclude!=''){
							$exclude_item = explode(',',$exclude);

							if($exclude_item == $list['qcopd_timelaps']){
								break;
							}
					...
					<li id="sld_bookmark_li_<?php echo $b; ?>" style="<?php echo ( isset($list['list_item_bg_color']) && !empty($list['list_item_bg_color']) ) ? 'background:'. esc_attr($list['list_item_bg_color']) : ''; ?>">
						...
						<?php else: ?>
						<a <?php if( $mask_url == 'on') { echo 'onclick="document.location.href = \''.$item_url.'\'; return false;"'; } ?> <?php echo esc_attr($qcopd_item_nofollow); ?> href="<?php echo esc_url($masked_url); ?>" <?php echo (isset($list['qcopd_item_newtab']) && $list['qcopd_item_newtab'] == 1) ? 'target="_blank"' : ''; ?>  >
						<?php endif; ?>

							<?php
								$iconClass = (isset($list['qcopd_fa_icon']) && trim($list['qcopd_fa_icon']) != "") ? esc_url($list['qcopd_fa_icon']) : "";
							...
							<!-- Link Text -->
							<div class="qcld-new-ca-content-outer">
						...
						<div class="bookmark-section bookmark-section-style-1">
								...
								<span data-post-id="<?php echo esc_attr($list['postid']); ?>" data-item-code="<?php echo esc_attr(trim($list['qcopd_timelaps'])); ?>" data-is-bookmarked="<?php echo ($bookmark); ?>" data-li-id="sld_bookmark_li_<?php echo esc_attr($b); ?>" class="bookmark-btn bookmark-on">
...
if($bookmark_list == "favorite" ){
	...
	while ( $list_query->have_posts() )
	{
		...
		<?php if( $style == "style-1" ) : 
			...
			$qcopd_list_custom_css .= "#qcopd-list-".$listId .'-'. get_the_ID().".style-1 .ca-menu li {
			    border-left: 10px solid ".@$conf['list_border_color'].";
			    background-color: ".@$conf['list_bg_color'].";
				box-shadow: 1px 1px 2px ".@$conf['item_bdr_color'].";
			}

			#qcopd-list-".$listId .'-'. get_the_ID().".style-1 .ca-menu li:hover {
			    background-color: ".@$conf['list_bg_color_hov'].";
				color: ".@$conf['list_txt_color_hov'].";
				box-shadow: 1px 1px 2px ".@$conf['item_bdr_color_hov'].";
			}

			#qcopd-list-".$listId .'-'. get_the_ID().".style-1 .ca-menu li .ca-main {
			  color: ".@$conf['list_txt_color'].";";
				if( isset($conf['list_bg_featured_color']) && $conf['item_title_font_size']!=''):
				$qcopd_list_custom_css .= "font-size: ".@$conf['item_title_font_size']." !important;";
				endif;

				if( isset($conf['list_bg_featured_color']) && $conf['item_title_line_height']!=''):
				$qcopd_list_custom_css .= "line-height: ".@$conf['item_title_line_height']." !important;";
				endif;
			$qcopd_list_custom_css .= "}";

			$qcopd_list_custom_css .= "#qcopd-list-".$listId .'-'. get_the_ID().".style-1 .ca-menu li:hover .ca-main {
			  color: ".@$conf['list_txt_color_hov'].";
			}

			#qcopd-list-".$listId .'-'. get_the_ID().".style-1 .ca-menu li .ca-sub {
			  color: ".@$conf['list_subtxt_color'].";";
				if( isset($conf['list_bg_featured_color']) && $conf['item_subtitle_font_size']!=''):
				$qcopd_list_custom_css .= "font-size: ".@$conf['item_subtitle_font_size']." !important;";
				endif;

				if( isset($conf['list_bg_featured_color']) && $conf['item_subtitle_line_height']!=''):
				$qcopd_list_custom_css .= "line-height: ".@$conf['item_subtitle_line_height']."!important;";
				endif;
			$qcopd_list_custom_css .= "}

			#qcopd-list-".$listId .'-'. get_the_ID().".style-1 .ca-menu li:hover .ca-sub {
			  color: ".@$conf['list_subtxt_color_hov'].";
			}

			#qcopd-list-".$listId .'-'. get_the_ID().".style-1 .ca-menu li .upvote-section .upvote-btn, #qcopd-list-".$listId .'-'. get_the_ID()." .ca-menu li .upvote-section .upvote-count{
			  color: ".@$conf['list_subtxt_color'].";
			}

			#qcopd-list-".$listId .'-'. get_the_ID().".style-1 .ca-menu li:hover .upvote-section .upvote-btn, #qcopd-list-".$listId .'-'. get_the_ID().".style-1 .ca-menu li:hover .upvote-section .upvote-count{
			  color: ".@$conf['list_subtxt_color_hov'].";
			}
			...
			if( isset($conf['list_bg_featured_color']) && !empty($conf['list_bg_featured_color'])){
				$qcopd_list_custom_css .= ".qcopd-list-wrapper .style-1 .featured-section {
        ...
        wp_add_inline_style( 'sld-css-style-1', $qcopd_list_custom_css );
					...
					<ul class="ca-menu" id="jp-list-<?php echo $SLD_QCOPD_DIRectory_instance_count.'-'.get_the_ID(); ?><?php echo (isset($cattabid)&&$cattabid!=''?'-'.$cattabid:''); ?>">
						...
						<li id="item-<?php echo esc_attr(get_the_ID() ."-". $count); ?>" style="<?php echo ( isset($list['list_item_bg_color']) && !empty($list['list_item_bg_color']) ) ? 'background:'. esc_attr($list['list_item_bg_color']) : ''; ?>">
							...
							<?php
								...
								if($main_click=='popup'){
									$masked_url = '#';
									$popContent = 'class="open-mpf-sld-more2 sld_load_more2" data-post-id="'.esc_attr(get_the_ID()).'" data-item-title="'.esc_html(trim($list['qcopd_item_title'])).'" data-item-link="'.esc_url($list['qcopd_item_link']).'" data-mfp-src="#sldinfo-'.get_the_ID() ."-". $count."-". esc_attr($list['qcopd_timelaps']).'"';
								}
							...
							<?php if(sld_is_youtube_video($item_url)): ?>
								...
								<a <?php echo $videoPopContent; ?> title="<?php echo esc_attr($list['qcopd_item_title']); ?>" href="<?php echo esc_url($list['qcopd_item_link']); ?>" <?php if($video_click!='popup'){ echo (isset($list['qcopd_item_newtab']) && $list['qcopd_item_newtab'] == 1) ? 'target="_blank"' : ''; echo 'data-itemurl="'.esc_url($list['qcopd_item_link']).'"'; } ?> data-mfp-src="#sldvideo-<?php echo $count; ?>" data-itemid="<?php echo esc_attr(get_the_ID()); ?>" data-videourl="<?php echo esc_url($list['qcopd_item_link']); ?>" data-itemsid="<?php echo esc_attr($list['qcopd_timelaps']); ?>" data-tag="<?php echo (isset($list['qcopd_tags'])?esc_attr($list['qcopd_tags']):'' ); ?>" data-subtitle="<?php echo esc_attr( $subtitle ); ?>">


								
							<?php elseif(sld_is_vimeo_video($item_url)): ?>
								<div id="sldvideo-<?php echo esc_attr($count); ?>" class="white-popup mfp-hide">
									<div class="sld_video">
										<?php echo esc_html__('Loading...', 'qc-opd'); ?>
									</div>
								</div>
								<a <?php echo $videoPopContent; ?> title="<?php echo esc_attr($list['qcopd_item_title']); ?>" href="<?php echo esc_url($list['qcopd_item_link']); ?>" <?php if($video_click!='popup'){ echo (isset($list['qcopd_item_newtab']) && $list['qcopd_item_newtab'] == 1) ? 'target="_blank"' : ''; echo 'data-itemurl="'.esc_url($list['qcopd_item_link']).'"'; } ?> data-mfp-src="#sldvideo-<?php echo $count; ?>" data-itemid="<?php echo esc_attr(get_the_ID()); ?>" data-videourl="<?php echo esc_url($list['qcopd_item_link']); ?>" data-itemsid="<?php echo esc_attr($list['qcopd_timelaps']); ?>" data-subtitle="<?php echo esc_attr($subtitle); ?>">
							...
							<?php else: ?>
								...
								<a title="<?php echo esc_attr($list['qcopd_item_title']); ?>" <?php if( $mask_url == 'on') { echo 'onclick="document.location.href = \''.$item_url.'\'; return false;"'; } ?> <?php echo esc_attr($qcopd_item_nofollow); ?> href="<?php echo esc_url($masked_url); ?>"
								<?php echo (isset($list['qcopd_item_newtab']) && $list['qcopd_item_newtab'] == 1) ? 'target="_blank"' : ''; ?> data-itemid="<?php echo esc_attr(get_the_ID()); ?>" data-itemurl="<?php echo esc_url($list['qcopd_item_link']); ?>" data-itemsid="<?php echo esc_attr($list['qcopd_timelaps']); ?>" data-tag="<?php echo (isset($list['qcopd_tags'])?esc_attr($list['qcopd_tags']):'' ); ?>" data-subtitle="<?php echo esc_attr( $subtitle ); ?>" <?php echo $popContent; ?>>
							...
							<?php if( $upvote == 'on' ) : ?>
								...
								<div class="upvote-section upvote-section-style1">
									<span data-post-id="<?php echo esc_attr(get_the_ID()); ?>" data-unique="<?php echo get_the_ID().'_'.($list['qcopd_timelaps']!=''?$list['qcopd_timelaps']:$count); ?>" data-item-title="<?php echo esc_attr(trim($list['qcopd_item_title'])); ?>" data-item-link="<?php echo esc_url($list['qcopd_item_link']); ?>" class="upvote-btn upvote-on">
										<i class="fa <?php echo esc_attr($sld_thumbs_up); ?>"></i>
									</span>
