...
jQuery(document).ready(function($){
	$("body").on('keyup', "input[name*='[qcopd_item_link]']", function(e){
		...
		if( value != '' ){
			...
			if(videoid != null) {
				var videoid_text = videoid[1].replace("shorts/", "");
				var videoid_text = videoid_text.replace("embed/", "");
			    var total_value = 'https://img.youtube.com/vi/'+videoid_text+'/mqdefault.jpg';
			}

			var res = $(this).closest('.cmb_metabox').find('#qcopd_item_img_link input').val().match(/google.com/g);
			var res_u = $(this).closest('.cmb_metabox').find('#qcopd_item_img_link input').val().match(/youtube.com/g);
			
			if($(this).closest('.cmb_metabox').find('#qcopd_item_img_link input').val()!=='' && ( res!==null || res_u!==null ) ){
