	...
	public function get_default_args() {
		return apply_filters(
			...
			array(
				...
				'data_delegate'       => null,
				'save_callback'       => null,
				'string-repeat-field' => __( 'Add New', 'qc-opd' ),
				'string-delete-field' => __( 'Remove Field', 'qc-opd' ),
			),
			get_class( $this )
		);
	}
	...
	public function html() {
		...
		<div class="cmb-file-wrap" <?php echo 'data-type="' . esc_attr( $data_type ) . '"'; ?>>
			...
			<button class="button cmb-file-upload <?php echo esc_attr( $this->get_value() ) ? 'hidden' : '' ?>">
				<?php esc_html_e( 'Add File', 'qc-opd' ); ?>
			</button>
		...
		*/
		...
		<div class="cmb-file-wrap" style="<?php echo esc_attr( $styles ); ?>" data-type="<?php echo esc_attr( $data_type ); ?>">
			...
			<button class="button cmb-file-upload <?php echo esc_attr( $this->get_value() ) ? 'hidden' : '' ?>" data-nonce="<?php echo wp_create_nonce( 'cmb-file-upload-nonce' ); ?>">
				<?php esc_html_e( 'Add Image', 'qc-opd' ); ?>
			</button>
...
class CMB_Select extends CMB_Field {
	...
	public function output_field() {
		...
		$none = is_string( $this->args['allow_none'] ) ? $this->args['allow_none'] : __( 'None', 'qc-opd' );
	...
	public function output_script() {
		...
		$options = wp_parse_args( $this->args['select2_options'], array(
			'placeholder' => __( 'Type to search', 'qc-opd' ),
			'allowClear'  => true,
		) );
...
class CMB_Group_Field extends CMB_Field {
	...
	public function get_default_args() {
		return array_merge(
			...
			array(
				'fields'              => array(),
				'string-repeat-field' => __( 'Add New', 'qc-opd' ),
				'string-delete-field' => __( 'Remove', 'qc-opd' ),
			)
		);
	}
	...
	public function display() {
		...
		if ( $this->args['repeatable'] ) {
			...
			?>
				<button class="button sld_ctm_btn1">
					<?php echo 'Collapse All'; ?>
				</button>
				<button class="button repeat-field sld_ctm_btn2">
	...
	public function html() {
		...
		<?php if ( $this->args['repeatable'] ) : ?>
			...
			<button class="sld_collapse">				
				<?php echo esc_html( 'Collapse' ); ?>
			</button>
		<?php endif; ?>
	...
	public function get_default_args() {
		return array_merge(
			...
			array(
				...
				'default_zoom'        => '8',
				'string-marker-title' => __( 'Drag to set the exact location', 'qc-opd' ),
			)
		);
	}
